...
import java.lang.reflect.{Field => JField, Method => JMethod, Modifier}
import java.util.regex.Pattern

import scala.collection.{
  AbstractSet, SortedSetLike, generic, immutable, mutable
}
import scala.reflect.NameTransformer._

/** Defines a finite set of values specific to the enumeration. Typically these
 *  values enumerate all possible forms something can take and provide a
 *  lightweight alternative to case classes.
 *
 *  Each call to a `Value` method adds a new unique value to the enumeration. To
 *  be accessible, these values are usually defined as `val` members of the
 *  evaluation.
 *
 ...
 *
 *  object WeekDay extends Enumeration { type WeekDay = Value val Mon, Tue, Wed,
 *  Thu, Fri, Sat, Sun = Value } import WeekDay._
 *
 *  def isWorkingDay(d: WeekDay) = ! (d == Sat || d == Sun)
 *
 *  WeekDay.values filter isWorkingDay foreach println } // output: // Mon //
 *  Tue // Wed // Thu // Fri }}}
 *
 *  @param initial
 *    The initial value from which to count the integers that identifies values
 *    at run-time.
 *  @author
 *    Matthias Zenger
 */
...
abstract class Enumeration(initial: Int) extends Serializable {
  ...
  /** Return a `Value` from this `Enumeration` whose name matches the argument
   *  `s`. The names are determined automatically via reflection.
   *
   *  @param s
   *    an `Enumeration` name
   *  @return
   ...
   *
   *  @param i
   *    An integer that identifies this value at run-time. It must be unique
   *    amongst all values of the enumeration.
   *  @return
  ...
  /** Creates a fresh value, part of this enumeration, called `name` and
   *  identified by the integer `i`.
   *
   *  @param i
   *    An integer that identifies this value at run-time. It must be unique
   *    amongst all values of the enumeration.
  ...
  abstract class Value extends Ordered[Value] with Serializable {
    ...
    /** a marker so we can tell whose values belong to whom come
     *  reflective-naming time
     */
  ...
  /** A class implementing the [[scala.Enumeration.Value]] type. This class can
   *  be overridden to change the enumeration's naming and integer
  ...
  /** A class for sets of values. Iterating through this set will yield values
   *  in increasing order of their ids.
   *
   *  @param nnIds
   *    The set of ids of values (adjusted so that the lowest value does not
   *    fall below zero), organized as a `BitSet`.
   */
  class ValueSet private[ValueSet] (private[this] var nnIds: immutable.BitSet)
    ...
    /** Creates a bit mask for the zero-adjusted ids in this set as a new array
     *  of longs
     */
