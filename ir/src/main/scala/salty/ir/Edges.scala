...
object Edge {
  ...
  def of(instr: ir.Instr): Seq[Edge] = instr match {
    ...
    case ir.Instr.If(cf, ef, v)     => Seq(Edge.Cf(cf), Edge.Ef(ef), Edge.Val(v))
    case ir.Instr.Switch(cf, ef, v) => Seq(Edge.Cf(cf), Edge.Ef(ef), Edge.Val(v))
    ...
    case ir.Instr.CaseEnd(cf, ef)   => Seq(Edge.Cf(cf), Edge.Ef(ef))
