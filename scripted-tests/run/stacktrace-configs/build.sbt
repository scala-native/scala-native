...
lazy val runStackTraceTest = taskKey[Unit]("Run stack trace test with current configuration")
runStackTraceTest := {
  ...
  if (result != 0) {
    throw new RuntimeException(s"Stack trace test failed with exit code $result")
  }
