...
object Conv extends Base[nir.Conv] {
  ...
  val Wordtoint = P("wordtoint".! map (_ => nir.Conv.Wordtoint))
  val Inttoword = P("inttoword".! map (_ => nir.Conv.Inttoword))
  val Bitcast   = P("bitcast".! map (_ => nir.Conv.Bitcast))
  override val parser: P[nir.Conv] =
    Trunc | Zext | Sext | Fptrunc | Fpext | Fptoui | Fptosi | Uitofp | Sitofp | Ptrtoint | Inttoptr | Wordtoint | Inttoword | Bitcast
}
