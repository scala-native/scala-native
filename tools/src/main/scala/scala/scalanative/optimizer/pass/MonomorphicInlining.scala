 ...
 */
class MonomorphicInlining(dispatchInfo: Map[String, Seq[Int]])(
    implicit top: Top)
    extends Pass {
  ...
  private def findImpl(meth: Method, clss: Class): String =
    if (meth.in == clss) {
      assert(
          meth.isConcrete,
          s"Method ${meth.name.id} belongs to a type observed at runtime. The method should be concrete!")
      s"${clss.name.id}::${meth.name.id}"
    } else {
      clss.allmethods
        .filter(m => m.isConcrete && m.name.id == meth.name.id) match {
        case Seq() =>
          ???
        case Seq(m) =>
          m.in match {
