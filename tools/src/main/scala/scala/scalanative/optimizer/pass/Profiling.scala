...
class Profiling(implicit top: ClassHierarchy.Top, fresh: Fresh) extends Pass {
  ...
  override def preDefn = {
    case defn @ Defn.Define(_, MethodRef(scope, meth), _, label +: insts) =>
      val newInsts =
        Seq(
          label,
          Let(
            Op.Call(log_callSig,
                    log_call,
                    Seq(Val.I32(scope.id), Val.I32(meth.id)),
                    Next.None))
        ) ++ insts
  ...
  override def preInst = {
    case l @ Let(_, Op.Load(ty, ptr)) =>
      Seq(
        Let(Op.Call(log_loadSig, log_load, Seq(), Next.None)),
        l
      )

    case l @ Let(_, Op.Store(ty, ptr, value)) =>
      Seq(
        Let(Op.Call(log_storeSig, log_store, Seq(), Next.None)),
        l
      )
        ...
        Seq(
          Let(
            Op.Call(log_methodSig,
                    log_method,
                    Seq(typeid, Val.I32(scope.id), Val.I32(meth.id)),
                    Next.None)),
          l
        )
      }
      ...
      val ifNullLog =
        Seq(
          Let(
            Op.Call(log_asSig,
                    log_as,
                    Seq(Val.I32(0), Val.I32(scope.id)),
                    Next.None)))
      val ifNonNullLog =
        (objTypeId: Val) =>
          Seq(
            Let(
              Op.Call(log_asSig,
                      log_as,
                      Seq(objTypeId, Val.I32(scope.id)),
                      Next.None)))
    ...
    case l @ Let(_, Op.Box(Ref(node), obj)) =>
      Seq(
        Let(Op.Call(log_boxSig, log_box, Seq(Val.I32(node.id)), Next.None)),
        l
      )
...
object Profiling extends PassCompanion {
  ...
  val log_callSig  = Type.Function(Seq(Type.I32, Type.I32), Type.Void)
  ...
  val log_classallocSig =
    Type.Function(Seq(Type.I32), Type.Void)
  ...
  val log_boxSig  = Type.Function(Seq(Type.I32), Type.Void)
