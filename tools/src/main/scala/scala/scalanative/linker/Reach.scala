  ...
  def reachExported(name: nir.Global.Top): Unit = {
    def isExported(defn: nir.Defn) = defn match {
      case nir.Defn.Define(attrs, nir.Global.Member(_, sig), _, _, _) =>
        // Exclude externForwarder generated by CFuncPtr.fromScalaFunction
        // These need to assigned to variable to be reachable
        (sig.isExtern || attrs.isExtern) &&
          sig != Reach.ExternForwarderSig
      case _ => false
    }
