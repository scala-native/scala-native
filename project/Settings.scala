...
object Settings {
  ...
  lazy val thisBuildSettings = Def.settings(
    ...
    // CI (non-Windows): ActionCache races on *.sbtdir.zip / parallel unpack.
    // Windows: keep disk cache — empty store forces packageBin jar rewrites and AccessDenied
    // when compilers still hold those jars. Windows sbtdir.zip races need Defender exclusions.
    Global / cacheStores := {
    ...
    // Windows CI: serialize Compile to reduce packageBin / ActionCache file races.
    Global / concurrentRestrictions ++= {
      if (isCI && isWindows) Seq(Tags.limit(Tags.Compile, 1))
      else Nil
    },
    Global / onLoad ~= { prev =>
      if (!isWindows) try {
