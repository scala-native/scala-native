...
object Settings {
  ...
  lazy val thisBuildSettings = Def.settings(
    ...
    // Try limit issues due to ActionCache races under ++ and publishLocal
    Global / cacheStores := {
      if (isCI) Seq(AggregateActionCacheStore.empty)
...
// Generate project name from project id.
  ...
  lazy val mimaSettings = Seq(
    ...
    mimaPreviousArtifacts ++= {
      ...
      val binCompatVersions = Set("0.5.11")
      binCompatVersions
        .map { version =>
  ...
  lazy val sbtPluginSettings = Def.settings(
    ...
    scriptedBatchExecution := {
      ...
      if (scala.util.Properties.isWin) false
      else scriptedBatchExecution.value
    },
    scriptedLaunchOpts := {
      def isSbt2 = CrossVersion
        .binarySbtVersion(scriptedSbt.value)
        .startsWith("2")
      // ActionCache opt-out via a disposable global.sbt when running under CI.
      val nestedCacheOptOut = Option.when(isCI && isSbt2) {
        val globalBase = baseDirectory.value / "target" / "scripted-sbt2-global"
        val versioned = globalBase / "1.0"
        IO.createDirectory(versioned)
        IO.write(
          versioned / "global.sbt",
          """|// Generated by scala-native Settings.sbtPluginSettings for CI scripted.
      ...
      scriptedLaunchOpts.value ++
        Seq(
          ...
          "-Dfile.encoding=UTF-8", // Windows uses Cp1250 as default
          // Avoid Windows named-pipe boot races in nested scripted sbt processes.
