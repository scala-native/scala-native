...
# Release v0.4.3-RC1 (2022-01-04)

We're happy to announce the release of Scala Native v0.4.3-RC1, which bring support for Scala 3. 
The minimal version of Scala supported by Scala Native is 3.1.0, due to fatal blockers in Scala 3.0.x.
This release does also contain performance-critical bug fixes when performing operations on Strings, 
...
we've decided that the 0.4.3 will be preceded by its release candidates. 
...
This behaviour however was created multiple issues in situations when an actual main method was derived. Because of that issue,
...
In Scala 2 these methods have been recommended and it is recommended to use empty-param variants of these methods, eg. `scalanative.unsafe.stackalloc[T]()` instead. 
This change was decided due to changes in how does Scala 3 interpret source code which can lead to dangerous, hard to debug runtime errors. 
