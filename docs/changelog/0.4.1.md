...
Our solution to this problem is introduction of linktime resolved expressions - special 
values using `@scala.scalanative.unsafe.resolvedAtLinktime` annotation. Each of such a values
is provides a stub in NIR, which would be replaced when performing linking, based on 
values defined in `scala.scalanative.build.NativeConfig`. 
Currently, linktime list of linktime resolve expressions is limited and users cannot create new ones, but it might change in the future
...
In case of usage `if` condition containg linktime resolved value, at the time of performing 
linking of NIR files, we can replace condition with simple boolean value `true/false`.
...
not need its defintion when building.  

On the snippet bellow you can see example of linktime conditional branching. Depending on
...
To check at runtime does you GC supports weak references you can use dedicated linktime resolved 
value `LinktimeInfo.isWeakReferenceSupported`
