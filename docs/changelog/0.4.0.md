...
We are happy to announce the next major release of Scala Native!

Release 0.4.0 brings many changes to Scala Native ecosystem, let's take a look at few of them. 

## Compiler changes
### Supported Scala version
The most important change introduced by this release is support for recent Scala versions, that means you're able to work
with the most recent versions of Scala 2 available at the moment of release - 2.12.12 and 2.13.4. 
Scala 2.11 is still support, what more it was enhanced by multiple features, i.e. handling of java default methods.
...
### NIR changes
With this major release we're forced to break backward compatibility with previous versions of SN. 
It means also means that any library published using version 0.4.0-M2 or older would not be compatible to use now.

We also have managed to fix multiple Native Intermiediete Representation related bugs, i.e the way Strings and Characters 
were stored or adding class literal allowing us to handle proper caching of `java.lang.Class` instances. 
Moreover, we are now collecting source code positions in NIR instructions, allowing us to add first class debug support in following releases.
...
Scala Native does not support full reflection support, although this feature might fix most of the issues that could occour 
in users code.
...
### Other
Multiple name mangling and runtime errors have been fixed. For more details check list of merged PRs


## Sbt plugin changes
### Native Config
We've decided to deprecate the current spare set of `NativeX` setting keys with single `NativeConfig` object. 
Existing 
...
### Other
* We've decided to no longer support sbt 0.13.x version - if your build still uses this legacy version you need to upgrade to version  1.1.6 or newer.
* Failures during the build of multiple projects using common jar were fixed. 
* The minimal version of Clang working with Scala Native is now 6.0
* You can also observe lower overall memory usage when compiling and linking.

## Native lib and unsafe package changes
### CFuncPtr changes
...
### Unsigned CSize
Since 0.4.0 version we are deprecation usage of ignored numeral types for methods expecting CSize which now is of type `ULong`.
...
We have updated our `java.lang.String` and `java.lang.Character` to be compatible with Unicode 13.0.0 specification,
allowing us to replace relatively old standard 6.3.0 used in JDK 8
Additionally we've implemented missing handling of `java.lang.String.to{Lower, Upper}Case` methods for 
special and language specific casing rules. Further one requires linking implementation for `Locale`. 
...
* In sbt config `NativeConfig` replaces spare set of `NativeX` key settings,
* Added new Garbage Collector - Concurrent Mark and Pararrell Sweep Garbage Collector, called Commix,
...
* Unicode handling updated to specification 13.0.0,
