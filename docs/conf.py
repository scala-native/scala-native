...
# -*- coding: utf-8 -*-
#
# Scala Native documentation build configuration file, created by
# sphinx-quickstart on Wed Sep 21 13:37:51 2016.
#
...
def lastSnapshotVersion():
    # Ensures we're inside a repo (raises if not)
    _ensure_repo()
    # Recosntructs version based on the last-published tag
...
def _git(cmd: list[str]) -> str:
    """Run a git command and return stripped stdout, or raise a helpful error."""
    try:
        out = subprocess.check_output(["git", *cmd], stderr=subprocess.STDOUT)
        return out.decode().strip()
    except subprocess.CalledProcessError as e:
        msg = e.output.decode().strip()
        raise RuntimeError(f"git {' '.join(cmd)} failed: {msg}") from e

def _ensure_repo() -> None:
    # Will fail with a clear error if we're not in a Git repo.
    _git(["rev-parse", "--git-dir"])

def tag_commit_sha(tag: str) -> str:
    """Full SHA of the commit pointed to by the tag (annotated or lightweight)."""
    # tag^{commit} resolves to the commit object even for annotated tags
    return _git(["rev-parse", f"{tag}^{{commit}}"])

def tag_commit_date_utc(tag: str) -> str:
    """UTC date (yyyyMMdd) of the tagged commit."""
    # %ct = UNIX epoch seconds of the commit time
    ts = int(_git(["log", "-1", "--format=%ct", tag]))
    return datetime.fromtimestamp(ts, tz=timezone.utc).strftime("%Y%m%d")

# The short X.Y version (binary version)
version = u'0.5'
last_stable_release = "0.5.10"
# The next version to be releaed, used to calculate snapshot versions
developed_version   = "0.5.11"
...
#
# html_title = u'Scala Native v0.1'
...
htmlhelp_basename = 'ScalaNativedoc'
...
latex_documents = [
    (master_doc, 'ScalaNative.tex', u'Scala Native Documentation',
     u'Denys Shabalin et al.', 'manual'),
]
...
man_pages = [
    (master_doc, 'scalanative', u'Scala Native Documentation',
     [author], 1)
]
...
texinfo_documents = [
    (master_doc, 'ScalaNative', u'Scala Native Documentation',
     author, 'ScalaNative', 'One line description of project.',
     'Miscellaneous'),
]
