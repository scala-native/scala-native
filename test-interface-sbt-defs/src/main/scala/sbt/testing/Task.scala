 ...
 *  supplying more tasks to the client. A framework can supply more tasks to the
 *  client in the returned an array of <em>Task</em>s (which can be empty if
...
trait Task {
  ...
  /** Scala.js / Scala Native specific: Same as basic
   ...
   *
   *  When running in a Scala Native/JavaScript environment, only this method
   *  will be called.
   */
  def execute(
   ...
   *
   *  @return
   *    the <code>TaskDef</code> that was used to request this
   *    <code>Task</code>.
   */
  def taskDef(): TaskDef
}
