 ...
 *  handler is for events intended to be consumed by the client software whereas
 *  the logger is for messages intended to be consumed by the client *user*
 *  (<em>i.e.</em>, a human).
 *
...
trait Logger {
   ...
   *
   *  @param t
   *    the <code>Throwable</code> containing the stack trace being logged
   */
