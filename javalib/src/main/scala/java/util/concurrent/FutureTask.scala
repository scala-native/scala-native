...
class FutureTask[V] extends RunnableFuture[V] {
  ...
  override def cancel(mayInterruptsIfRunning: Boolean): Boolean = sync.innerCancel(mayInterruptsIfRunning)

  override def get(): V = sync.innerGet

  override def get(timeout: Long, unit: TimeUnit): V = sync.innerGet(unit.toNanos(timeout))
  ...
  private final class Sync(val callable: Callable[V]) extends AbstractQueuedSynchronizer {
    ...
    private def ranOrCancelled(state: Int): Boolean = (state & (RAN | CANCELLED)) != 0

    override protected def tryAcquireShared(ignore: Int): Int = if(innerIsDone) 1 else -1
    ...
    def innerIsDone: Boolean = ranOrCancelled(getState) && runner == null.asInstanceOf[Thread]
