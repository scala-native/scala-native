...
class CopyOnWriteArrayList[E <: AnyRef] private (
      ...
      obj match {
        case obj: List[_] =>
          ...
          this.scalaOps.forall(elem =>
            oIter.hasNext() && Objects.equals(elem, oIter.next())) && !oIter
            .hasNext()
        case _ => false
      }
    }
  ...
  // SN port:
  // innerPush() is emulating javascript jsArray.push which adds to the
  // _end_ of the list.
  // This is unlike & opposite of  j.u.LinkdedList or j.u.ArrayDeque
  // which add to the beginning of the list.
  ...
  protected def innerRemove(index: Int): E = {
    val i = if (index >= 0) index else (inner.size() + index - 1)
    inner.remove(i)
  }

  protected def innerRemoveMany(index: Int, count: Int): Unit = {
    val i = if (index >= 0) index else (inner.size() + index - 1)
    inner.removeRange(i, index + count)
  }

  protected def copyIfNeeded(): Unit = {
    if (requiresCopyOnWrite) {
      inner = inner.clone().asInstanceOf[ArrayList[E]]
      requiresCopyOnWrite = false
    }
  }

  protected def innerSnapshot(): Array[E] = {
    requiresCopyOnWrite = true
    inner.toArray().asInstanceOf[Array[E]]
  }
  ...
  private class CopyOnWriteArrayListView(fromIndex: Int,
    ...
    override protected def innerSnapshot(): Array[E] =
      self.innerSnapshot()
...
private class CopyOnWriteArrayListIterator[E](arraySnapshot: Array[E],
                                              i: Int,
                                              start: Int,
                                              end: Int)
