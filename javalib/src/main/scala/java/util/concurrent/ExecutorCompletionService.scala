...
class ExecutorCompletionService[V](val executor: Executor, val completionQueue: BlockingQueue[Future[V]] = new LinkedBlockingQueue[Future[V]]()) extends CompletionService[V] {
  ...
  private class QueueingFuture(val task: RunnableFuture[V]) extends FutureTask[Void](task, null) {
  ...
  override def poll(timeout: Long, unit: TimeUnit): Future[V] = completionQueue.poll(timeout, unit)
