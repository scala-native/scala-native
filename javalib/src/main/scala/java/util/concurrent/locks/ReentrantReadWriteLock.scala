...
class ReentrantReadWriteLock extends ReadWriteLock with java.io.Serializable { self =>

  import ReentrantReadWriteLock._

  private final val readerLock: ReadLock = new ReentrantReadWriteLock.ReadLock()

  private final val writerLock: WriteLock = new ReentrantReadWriteLock.WriteLock()
...
object ReentrantReadWriteLock {
  ...
  abstract class Sync extends AbstractQueuedSynchronizer {
    ...
    override protected[ReentrantReadWriteLock] final def isHeldExclusively: Boolean = getExclusiveOwnerThread == Thread.currentThread()
