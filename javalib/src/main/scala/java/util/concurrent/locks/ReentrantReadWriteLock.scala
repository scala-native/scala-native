...
object ReentrantReadWriteLock {
  ...
  class WriteLock private (final private val sync: ReentrantReadWriteLock.Sync)
     ...
     *  </ul>
     *
     *  <p>If the write lock is acquired by the current thread then the lock
     ...
     *  setting the write lock hold count to one. Even when this lock has been
     *  set to use a fair ordering policy, a call to {@code tryLock()}
     *  <em>will</em> immediately acquire the lock if it is available, whether
     *  or not other threads are currently waiting for the write lock. This
     *  &quot;barging&quot; behavior can be useful in certain circumstances,
     *  even though it breaks fairness. If you want to honor the fairness
     *  setting for this lock, then use {@link #tryLock(long, TimeUnit)
     ...
     *    thread, or the write lock was already held by the current thread; and
     *    {@code false} otherwise.
     */
    override def tryLock(): Boolean = sync.tryWriteLock

    /** Acquires the write lock if it is not held by another thread within the
     *  given waiting time and the current thread has not been {@linkplain
     *  Thread#interrupt interrupted}.
     *
     *  <p>Acquires the write lock if neither the read nor write lock are held
     *  by another thread and returns immediately with the value {@code true},
     *  setting the write lock hold count to one. If this lock has been set to
     *  use a fair ordering policy then an available lock <em>will not</em> be
     *  acquired if any other threads are waiting for the write lock. This is in
     *  contrast to the {@link #tryLock()} method. If you want a timed {@code
     *  tryLock} that does permit barging on a fair lock then combine the timed
     *  and un-timed forms together:
     *
     *  <pre> {@code if (lock.tryLock() || lock.tryLock(timeout, unit)) { ...
     *  }}</pre>
     *
     ...
     *  </ul>
     *
     *  <p>If the write lock is acquired then the value {@code true} is returned
     ...
     *  instance. <p>The returned {@link Condition} instance supports the same
     ...
     *  <ul>
     *
     *  <li>If this write lock is not held when any {@link Condition} method is
     *  called then an {@link IllegalMonitorStateException} is thrown. (Read
     *  locks are held independently of write locks, so are not checked or
     *  affected. However it is essentially always an error to invoke a
     *  condition waiting method when the current thread has also acquired read
     *  locks, since other threads that could unblock it will not be able to
     *  acquire the write lock.)
     *
     *  <li>When the condition {@linkplain Condition#await() waiting} methods
     *  are called the write lock is released and, before they return, the write
     *  lock is reacquired and the lock hold count restored to what it was when
