...
class TreeMap[K, V] private (tree: RB.Tree[K, V])(implicit
  ...
  def descendingKeySet(): NavigableSet[K] = {
    new TreeSet.DescendingProjection(
      tree,
      null.asInstanceOf[K],
      RB.NoBound,
      null.asInstanceOf[K],
  ...
  def descendingMap(): NavigableMap[K, V] = {
    new TreeMap.DescendingProjection(
      tree,
      null.asInstanceOf[K],
      RB.NoBound,
      null.asInstanceOf[K],
...
private object TreeMap {
  private class ProjectedEntrySet[K, V](
      tree: RB.Tree[K, V],
      lowerBound: K,
    ...
    private def isWithinBounds(key: Any): Boolean =
      RB.isWithinLowerBound(key, lowerBound, lowerKind) && RB
  ...
  private abstract class AbstractProjection[K, V](
    ...
    protected def subMapGeneric(
        newFromKey: K = null.asInstanceOf[K],
  ...
  private final class Projection[K, V](
    ...
    def descendingMap(): NavigableMap[K, V] = {
      new DescendingProjection(tree, toKey, toBoundKind, fromKey, fromBoundKind)
    }
  }
