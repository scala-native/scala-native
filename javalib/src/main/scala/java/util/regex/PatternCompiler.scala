...
private[regex] object PatternCompiler {
  ...
  /** Scala Native uses the standalone regexp engine with full Unicode support. */
  private[regex] object Support {
    @inline def supportsUnicode: Boolean = true
    @inline def supportsSticky: Boolean = true
    @inline def supportsDotAll: Boolean = true
    @inline def supportsIndices: Boolean = featureTest("d")
    @inline def enableUnicodeCaseInsensitive: Boolean = true
    @inline def enableUnicodeCharacterClassesAndLookBehinds: Boolean = true
  }
  ...
  private lazy val predefinedPCharacterClasses: engine.Dictionary[CompiledCharClass] = {
    ...
    mapSet(result, "IsPunctuation", posP("P"))
...
private final class PatternCompiler(private val pattern: String, private var flags: Int) {
  ...
  private def compileTopLevelOrInsideGroup(insideGroup: Boolean): String = {
    ...
    while (pIndex != len) {
      ...
      dispatchCP match {
        ...
        case _ =>
          val compiledToken = dispatchCP match {
            case '\\' => compileEscape()
          ...
          result += compileRepeater(compiledToken)
      }
    }
  ...
  private def compileRepeater(compiledToken: String): String = {
    val pattern = this.pattern // local copy
    val len = pattern.length()
      ...
      if (pIndex != len) {
        pattern.charAt(pIndex) match {
          case '+' =>
            ...
            pIndex += 1
            buildPossessiveQuantifier(wrappedToken, baseRepeater)
          case '?' =>
            // Lazy quantifier
            pIndex += 1
   ...
   *  a greedy quantifier.
   */
  private def buildPossessiveQuantifier(compiledToken: String, baseRepeater: String): String = {
    /* Standalone engine supports possessive quantifiers (see tc39 atomic ops proposal). */
  ...
  private def canonicalizeScriptName(scriptName: String): String = {
    ...
    engine.dictGetOrElseUpdate(canonicalizedScriptNameCache, canonical) { () =>
      if (!engine.validateScriptName(canonical))
  ...
  private def compileCharacterClass(): String = {
    ...
    while (pIndex != len) {
      ...
      pattern.codePointAt(pIndex) match {
        case ']' =>
          pIndex += 1
