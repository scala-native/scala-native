...
abstract class AsynchronousFileChannel extends AsynchronousChannel {
  ...
  final def lock[A](attachment: A, handler: CompletionHandler[FileLock, _ >: A]): Unit =
  ...
  def read[A](dst: ByteBuffer, position: Long, attachment: A, handler: CompletionHandler[Integer, _ >: A]): Unit
  def read(dst: ByteBuffer, position: Long): Future[Integer]
  def write[A](src: ByteBuffer, position: Long, attachment: A, handler: CompletionHandler[Integer, _ >: A]): Unit
