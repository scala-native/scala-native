    ...
    val fun: FunctionPtr4[CString, Ptr[stat], Int, Ptr[FTW], Int] =
      (fileBuffer: CString, sb: Ptr[stat], flag: Int, s: Ptr[FTW]) => {
        ...
        def attributes = new BasicFileAttributes {
          ...
          val creationTime =
            FileTime.from((!sb).st_ctime_nsec.toLong, NANOSECONDS)
          val lastAccessTime =
            FileTime.from((!sb).st_atime_nsec.toLong, NANOSECONDS)
          val lastModifiedTime =
            FileTime.from((!sb).st_mtime_nsec.toLong, NANOSECONDS)
        ...
        val visitResult =
          flag match {
            ...
            case `FTW_DNR` =>
              fv.postVisitDirectory(
                path,
                new IOException("cannot read from directory"))
            case `FTW_NS` =>
              fv.postVisitDirectory(path, new IOException("cannot stat inode"))
            case `FTW_DP` =>
              fv.postVisitDirectory(
                path,
                new IOException(
                  "visiting a symbolic link with FTW_PHYS flag set")) // should not happen
            case `FTW_SLN` =>
              fv.visitFileFailed(path, new IOException("invalid symlink"))
            case _ =>
              fv.postVisitDirectory(path,
                                    new IOException(s"unkown flag: $flag"))
          }
        convert(visitResult)
      }
