...
private[lang] object NumberParser {
  ...
  // ToDo reimplement
  @inline private def parseDblImpl(s: _String, e: scala.Int): scala.Double =
    ???
  @inline private def parseFltImpl(s: _String, e: scala.Int): scala.Float = ???

  @inline private def initialParse(
      _s: _String, _length: scala.Int): StringExponentPair = {
    var s            = _s
    var length       = _length
    var negative     = false
    var c: Char      = 0
  ...
  private def parseDblName(namedDouble: _String, length: Int): scala.Double = {
  ...
  private def parseFltName(namedFloat: _String, length: Int): scala.Float = {
    if ((length != 3) && (length != 4) && (length != 8) && (length != 9)) {
      throw new NumberFormatException()
    } else {
      var negative = false
  ...
  private def parseAsHex(s: _String): scala.Boolean = {
    val length = s.length
    if (length < 2) {
      false
    } else {
  ...
  def parseDouble(_s: _String): scala.Double = {
    val s      = _s.trim()
    val length = s.length()
    if (length == 0) {
      throw new NumberFormatException(s)
  ...
  def parseFloat(_s: _String): scala.Float = {
    val s      = _s.trim()
    val length = s.length()
    if (length == 0) {
      throw new NumberFormatException(s)
