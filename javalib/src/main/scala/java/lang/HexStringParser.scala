...
private object HexStringParser {
  ...
  def parseDouble(hexString: String): scala.Double = {
    val parser = new HexStringParser(
        DOUBLE_EXPONENT_WIDTH, DOUBLE_MANTISSA_WIDTH)
    val result = parser.parse(hexString, isDouble = true)
    Double.longBitsToDouble(result)
  }

  def parseFloat(hexString: String): scala.Float = {
    val parser = new HexStringParser(
        FLOAT_EXPONENT_WIDTH, FLOAT_MANTISSA_WIDTH)
    val result = parser.parse(hexString, isDouble = false).toInt
...
private class HexStringParser(
  ...
  private var abandonedNumber: String = ""

  private def parse(hexString: String, isDouble: scala.Boolean): scala.Long = {
    val matcher = HexStringParser.PATTERN.matcher(hexString)
    if (!matcher.matches()) {
  ...
  private def parseHexSign(signStr: String) {
    this.sign =
      if (signStr == "-") 1
      else 0
  }

  private def parseExponent(_exponentStr: String) {
  ...
  private def parseMantissa(significantStr: String) {
  ...
  private def getNormalizedSignificand(strIntegerPart: String,
                                       strDecimalPart: String): String = {
    val significand = (strIntegerPart + strDecimalPart).replaceFirst("^0+", "")
    if (significand.length == 0) "0"
    else significand
  }

  private def getOffset(_strIntegerPart: String, strDecimalPart: String): Int = {
    val strIntegerPart = _strIntegerPart.replaceFirst("^0+", "")
