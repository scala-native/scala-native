...
object Float {
  ...
  @inline def parseFloat(s: _String): scala.Float =
    NumberParser.parseFloat(s)

  @inline def sum(a: scala.Float, b: scala.Float): scala.Float =
    a + b

  def toHexString(f: scala.Float): _String =
    if (f != f) {
      "NaN"
  ...
  @inline def toString(f: scala.Float): _String =
    NumberConverter.convert(f)

  @inline def valueOf(s: _String): Float =
