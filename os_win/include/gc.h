...
#pragma message(                                                               \
    "Warning: Expecting _WIN64 for x64 targets! Notice the leading underscore!")
#endif
...
#define GC_GENERAL_REGISTER_DISAPPEARING_LINK(link, obj)                       \
    GC_general_register_disappearing_link(                                     \
        link, GC_base((/* no const */ void *)(obj)))
...
GC_API int GC_CALL GC_get_stack_base(struct GC_stack_base *) GC_ATTR_NONNULL(1);
...
typedef int(GC_CALLBACK *GC_has_static_roots_func)(const char * /* dlpi_name */,
                                                   void * /* section_start */,
                                                   size_t /* section_size */);
...
#define GC_DATASTART                                                           \
    ((GC_word)__data_start__ < (GC_word)__bss_start__ ? (void *)__data_start__ \
                                                      : (void *)__bss_start__)
#define GC_DATAEND                                                             \
    ((GC_word)__data_end__ > (GC_word)__bss_end__ ? (void *)__data_end__       \
                                                  : (void *)__bss_end__)
#else
extern int _data_start__[], _data_end__[], _bss_start__[], _bss_end__[];
#define GC_DATASTART                                                           \
    ((GC_word)_data_start__ < (GC_word)_bss_start__ ? (void *)_data_start__    \
                                                    : (void *)_bss_start__)
#define GC_DATAEND                                                             \
    ((GC_word)_data_end__ > (GC_word)_bss_end__ ? (void *)_data_end__          \
                                                : (void *)_bss_end__)
#endif /* !__x86_64__ */
...
#define GC_INIT_CONF_ROOTS                                                     \
    (void)((GC_word)__data_start < (GC_word)_etext &&                          \
               ...
               ? (__end__ != 0 ? (GC_add_roots(__dso_handle, __end__), 0)      \
               ...
               : __data_start != 0                                             \
                     ? (__end__ != 0                                           \
                            ...
                            : (GC_word)__data_start < (GC_word)_end            \
                                  ? (GC_add_roots(__data_start, _end), 0)      \
                                  : 0)                                         \
                     : 0)
#else
...
#define GC_malloc_stubborn(a) (*GC_amiga_allocwrapper_do)(a, GC_malloc_stubborn)
