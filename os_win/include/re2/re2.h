...
// Otherwise the StringPiece form would be sufficient.
    ...
    // Returns the program size, a very approximate measure of a regexp's
    // "cost".
    ...
    //    T               (where "bool T::ParseFrom(const char*, size_t)"
    //    exists)
  ...
  public:
    ...
    template <typename... A>
    static bool PartialMatch(const StringPiece &text, const RE2 &re,
                             A &&... a) {
    ...
    // or
    // '+' operator). Regexps with "{N}" constructions are not affected, as
    // those
    ...
    bool Match(const StringPiece &text, size_t startpos, size_t endpos,
               Anchor anchor, StringPiece *match, int nmatch) const;
    ...
    // be sucessful.
    bool Rewrite(string *out, const StringPiece &rewrite,
    ...
    class Options {
      public:
        ...
        //
        //   utf8             (true)  text and pattern are UTF-8; otherwise
        //   Latin-1
        ...
        //   case_sensitive   (true)  match is case-sensitive (regexp can
        //   override
        ...
        //   one_line         (false) ^ and $ only match beginning and end of
        //   text
        //
        ...
        // and perhaps 2.5 MB per DFA (DFA state sizes vary by regexp; RE2 does
        // a
        ...
        // That makes 4 DFAs:
        //
        //   forward, first-match    - used for UNANCHORED or ANCHOR_LEFT
        //   searches
        ...
        //   reverse, longest-match  - used as second phase for unanchored
        //   searches
        //
        ...
        Options()
            : encoding_(EncodingUTF8), posix_syntax_(false),
              longest_match_(false), log_errors_(true),
              max_mem_(kDefaultMaxMem), literal_(false), never_nl_(false),
              dot_nl_(false), never_capture_(false), case_sensitive_(true),
              perl_classes_(false), word_boundary_(false), one_line_(false) {}
  ...
  private:
    ...
    bool DoMatch(const StringPiece &text, Anchor anchor, size_t *consumed,
...
template <class T> class _RE2_MatchObject {
  public:
...
#undef MAKE_PARSER
    ...
    template <class T>
    Arg(T *p) : arg_(p), parser_(_RE2_MatchObject<T>::Parse) {}
    template <class T> Arg(T *p, Parser parser) : arg_(p), parser_(parser) {}
