...
inline void *&GC_obj_link(void *p) { return *reinterpret_cast<void **>(p); }
...
inline size_t GC_round_up_uncollectable(size_t n) {
    return ((n + GC_byte_alignment - 1) / GC_byte_alignment) *
           GC_word_alignment;
}

template <int dummy> class GC_aux_template {
  public:
...
template <int dummy> size_t GC_aux_template<dummy>::GC_bytes_recently_freed = 0;
...
void *GC_aux_template<dummy>::GC_out_of_line_malloc(size_t nwords, int kind) {
    ...
    GC_non_gc_bytes += GC_uncollectable_bytes_recently_allocd;
...
template <int dummy> class single_client_gc_alloc_template {
  public:
...
template <int dummy> class single_client_traceable_alloc_template {
  public:
    static void *allocate(size_t n) {
...
template <int dummy> class gc_alloc_template {
  public:
    static void *allocate(size_t n) { return GC_malloc(n); }
    static void *ptr_free_allocate(size_t n) { return GC_malloc_atomic(n); }
...
template <int dummy> class traceable_alloc_template {
  public:
    ...
    class simple_alloc<T, alloc> {                                             \
      public:                                                                  \
        static T *allocate(size_t n) {                                         \
            return 0 == n ? 0 : reinterpret_cast<T *>(                         \
                                    alloc::ptr_free_allocate(n * sizeof(T)));  \
        }                                                                      \
        ...
        static void deallocate(T *p, size_t n) {                               \
            if (0 != n)                                                        \
                alloc::ptr_free_deallocate(p, n * sizeof(T));                  \
        }                                                                      \
...
template <class _Tp> struct _Alloc_traits<_Tp, gc_alloc> {
    static const bool _S_instanceless = true;
    typedef simple_alloc<_Tp, gc_alloc> _Alloc_type;
    typedef __allocator<_Tp, gc_alloc> allocator_type;
};

inline bool operator==(const gc_alloc &, const gc_alloc &) { return true; }

inline bool operator!=(const gc_alloc &, const gc_alloc &) { return false; }

template <class _Tp> struct _Alloc_traits<_Tp, single_client_gc_alloc> {
    static const bool _S_instanceless = true;
...
template <class _Tp> struct _Alloc_traits<_Tp, traceable_alloc> {
    static const bool _S_instanceless = true;
    typedef simple_alloc<_Tp, traceable_alloc> _Alloc_type;
    typedef __allocator<_Tp, traceable_alloc> allocator_type;
};

inline bool operator==(const traceable_alloc &, const traceable_alloc &) {
    return true;
}
...
template <class _Tp> struct _Alloc_traits<_Tp, single_client_traceable_alloc> {
    static const bool _S_instanceless = true;
