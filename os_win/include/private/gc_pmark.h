    ...
    do {                                                                       \
        ...
        if (_descr != 0) {                                                     \
            ...
            if ((word)mark_stack_top >= (word)(mark_stack_limit)) {            \
                mark_stack_top =                                               \
                    GC_signal_mark_stack_overflow(mark_stack_top);             \
            }                                                                  \
...
#define PUSH_CONTENTS(current, mark_stack_top, mark_stack_limit, source)       \
    do {                                                                       \
        ...
        PUSH_CONTENTS_HDR(current, mark_stack_top, mark_stack_limit, source,   \
                          my_hhdr, TRUE);                                      \
    } while (0)
...
#define SET_MARK_BIT_EXIT_IF_SET(hhdr, bit_no)                                 \
    { /* cannot use do-while(0) here */                                        \
        ...
        OR_WORD_EXIT_IF_SET(mark_word_addr,                                    \
                            (word)1                                            \
                                << modWORDSZ(bit_no)); /* contains "break" */  \
    }
...
#define TRACE(source, cmd)                                                     \
    if (GC_trace_addr != 0 && (ptr_t)(source) == GC_trace_addr)                \
    cmd
#define TRACE_TARGET(target, cmd)                                              \
    if (GC_trace_addr != 0 && (target) == *(ptr_t *)GC_trace_addr)             \
    cmd
#else
#define TRACE(source, cmd)
...
#define PUSH_CONTENTS_HDR(current, mark_stack_top, mark_stack_limit, source,   \
                          hhdr, do_offset_check)                               \
    do {                                                                       \
        size_t displ =                                                         \
            HBLKDISPL(current); /* Displacement in block; in bytes. */         \
        ...
        if (EXPECT((gran_offset | byte_offset) != 0, FALSE)) {                 \
            if ((hhdr->hb_flags & LARGE_BLOCK) != 0) {                         \
                ...
                size_t obj_displ =                                             \
                    GRANULES_TO_BYTES(gran_offset) + byte_offset;              \
                if (do_offset_check && !GC_valid_offsets[obj_displ]) {         \
                    GC_ADD_TO_BLACK_LIST_NORMAL(current, source);              \
                    break;                                                     \
                }                                                              \
...
#define GC_PUSH_ONE_HEAP(p, source, mark_stack_top)                            \
    do {                                                                       \
        ...
        if ((word)(p) >= (word)GC_least_plausible_heap_addr &&                 \
            (word)(p) < (word)GC_greatest_plausible_heap_addr)                 \
            mark_stack_top =                                                   \
                GC_mark_and_push((void *)(p), mark_stack_top,                  \
