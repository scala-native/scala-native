...
#define mach_type_known
    ...
    !defined(__CEGCC__) && !defined(SYMBIAN)
#define NOSYS
#define mach_type_known
#endif
#endif
...
#if GC_GNUC_PREREQ(2, 8) && !defined(__INTEL_COMPILER) &&                      \
    !defined(__PATHCC__) && !defined(__FUJITSU) /* for FX10 system */          \
    && !(defined(POWERPC) && defined(DARWIN))   /* for MacOS X 10.3.9 */       \
    && !defined(RTEMS) && !defined(__clang__)   /* since no-op in clang (3.0) */
...
extern int etext[];
#define DATASTART                                                              \
    ((ptr_t)((((word)(etext)) + 0x3fffff) & ~0x3fffff) + ((word)(etext)&0xfff))
...
#define DYNAMIC_LOADING
#include <features.h>
#if defined(__GLIBC__) && __GLIBC__ >= 2 || defined(PLATFORM_ANDROID)
#define SEARCH_FOR_DATA_START
#else
extern char **__environ;
#define DATASTART ((ptr_t)(&__environ))
...
#define DATAEND ((ptr_t)(_end))
    ...
    !(GC_GNUC_PREREQ(4, 8) || GC_CLANG_PREREQ(3, 2) || __ANDROID_API__ >= 18)
...
#define DATASTART                                                              \
    ((ptr_t)((((word)(etext) + 0x3ffff) & ~0x3ffff) + ((word)(etext)&0xffff)))
#define DATAEND ((ptr_t)(edata))
#define GC_HAVE_DATAREGION2
#define DATASTART2                                                             \
    (_DYNAMIC_LINKING ? (ptr_t)(((word)_gp + 0x8000 + 0x3ffff) & ~0x3ffff)     \
                      : (ptr_t)edata)
#define DATAEND2 ((ptr_t)(end))
#define ALIGNMENT 4
#endif
...
#ifndef GETPAGESIZE
#if defined(SOLARIS) || defined(IRIX5) || defined(LINUX) || defined(NETBSD) || \
    defined(FREEBSD) || defined(HPUX)
#include <unistd.h>
#endif
#define GETPAGESIZE() (unsigned)getpagesize()
#endif
...
#define GC_OPENBSD_UTHREADS 1
    ...
    defined(DGUX) || defined(BSD) || defined(HURD) || defined(AIX) ||          \
    defined(DARWIN) || defined(OSF1)
...
#define DEFAULT_VDB
      ...
      (defined(DARWIN) || defined(HURD) || defined(OPENBSD) ||                 \
       defined(ARM32) || defined(MIPS) || defined(AVR32) || defined(OR1K) ||   \
       defined(NIOS2))) ||                                                     \
...
#define IF_CANCEL(x) /* empty */
    ...
    ((defined(GC_PTHREADS) && !defined(NACL) && !defined(GC_WIN32_PTHREADS) && \
...
#if defined(CAN_HANDLE_FORK) && !defined(CAN_CALL_ATFORK) && !defined(HURD) && \
...
#define GET_MEM(bytes)                                                         \
    HBLKPTR(real_malloc(SIZET_SAT_ADD(bytes, GC_page_size)) + GC_page_size - 1)
#elif defined(OS2)
...
#define GET_MEM(bytes)                                                         \
    HBLKPTR(GC_MacTemporaryNewPtr(SIZET_SAT_ADD(bytes, GC_page_size), true) +  \
            GC_page_size - 1)
#else
#define GET_MEM(bytes)                                                         \
    HBLKPTR(NewPtrClear(SIZET_SAT_ADD(bytes, GC_page_size)) + GC_page_size - 1)
#endif
...
#define GET_MEM(bytes)                                                         \
    HBLKPTR((size_t)GC_amiga_get_mem(SIZET_SAT_ADD(bytes, GC_page_size)) +     \
            GC_page_size - 1)
