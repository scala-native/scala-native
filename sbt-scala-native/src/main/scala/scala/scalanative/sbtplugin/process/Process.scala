...
trait ProcessExtra {
  ...
  implicit def buildersToProcess[T](builders: Seq[T])(
      implicit convert: T => SourcePartialBuilder): Seq[SourcePartialBuilder] =
    applySeq(builders)
...
object Process extends ProcessExtra {
  ...
  def apply(builder: JProcessBuilder): ProcessBuilder =
    new SimpleProcessBuilder(builder)
  ...
  def apply(name: String, exitValue: => Int): ProcessBuilder =
    new DummyProcessBuilder(name, exitValue)

  def cat(file: SourcePartialBuilder,
          files: SourcePartialBuilder*): ProcessBuilder =
    cat(file :: files.toList)
...
trait SourcePartialBuilder {
  ...
  def #>(out: => OutputStream): ProcessBuilder =
    #>(new OutputStreamBuilder(out))
  def #>(b: ProcessBuilder): ProcessBuilder =
    new PipedProcessBuilder(toSource, b, false, ExitCodes.firstIfNonzero)
...
trait SinkPartialBuilder {
  ...
  def #<(b: ProcessBuilder): ProcessBuilder =
    new PipedProcessBuilder(b, toSink, false, ExitCodes.firstIfNonzero)
...
final class ProcessIO(val writeInput: OutputStream => Unit,
  ...
  def withError(process: InputStream => Unit): ProcessIO =
    new ProcessIO(writeInput, processOutput, process, inheritInput)
  def withInput(write: OutputStream => Unit): ProcessIO =
    new ProcessIO(write, processOutput, processError, inheritInput)
}
trait ProcessLogger {
