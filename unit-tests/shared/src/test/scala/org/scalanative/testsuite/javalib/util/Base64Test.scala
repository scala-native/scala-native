...
class Base64Test {
  ...
  @Test def encodeOneArray(): Unit = {
    ...
    assertEquals(encodedResults.length, results.size)
    for (((name, enc), exp) <- results.zip(encodedResults)) {
      assertEquals(exp, new String(enc, ISO_8859_1))
    }
  }
  ...
  @Test def encodeByteBuffer(): Unit = {
    for (((name, in, enc), exp) <- encoders.zip(encodedResults)) {
      ...
      assertEquals(exp, new String(result2.array(), ISO_8859_1))
    }
  }
  ...
  @Test def decodeFromArray(): Unit = {
    for ((encoded, (decoder, in)) <- encodedResults.zip(decodersAndInputs)) {
      ...
      assertEquals(in, new String(result, ISO_8859_1))
    }
  }
  ...
  @Test def decodeInputStream(): Unit = {
    for ((encoded, (decoder, expected)) <- encodedResults.zip(
      ...
      instream.close()
      assertEquals(expected, new String(read, ISO_8859_1))
    }
  }
  ...
  private def decodeInputStream(
      decoder: Decoder,
      input: String,
      dangling: Array[Byte] = Array.empty
  ): String = {
  ...
  def getDecoderAndInput(text: String): (Decoder, String) = {
    val decoder = {
      if (text.contains("basic")) basic
        ...
        throw new IllegalArgumentException(s"no decoder found in string $text")
    }
    val input = text.dropWhile('%' != _).tail.takeWhile('%' != _)
    decoder -> input
  }
