...
class ConcurrentHashMapTest extends MapTest {
  ...
  @Test def testIteratorsAreWeaklyConsistent(): Unit = {
    ...
    for (_ <- 0 until 30) {
      ...
      while (entryIter.hasNext()) {
        ...
        assertTrue(
          s"duplicate iteration of key '$key'",
          encounteredKeys.add(key)
        )
        assertTrue(s"unexpected key '$key'", possibleValuesFor.contains(key))
        assertTrue(
          s"unexpected value '$value' for key '$key'",
  ...
  @Test def addOnKeySetView(): Unit = {
    ...
    assertFalse(keySet.add("ONE"))
    assertEquals(
      "Adding an existing key does not alter the value",
      42,
      map.get("ONE")
    )
  }
  ...
  @Test def toStringOnKeySetView(): Unit = {
    ...
    assertTrue(
      s"toString should print keys, but actual: $str",
      str == "[a, b]" || str == "[b, a]"
    )
  }
}
