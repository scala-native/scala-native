...
class ProcessTest {
  ...
  @Test def testGitLsFilesUsingJavaProcessStart(
    ...
    assertTrue(
      "`git ls-files` should exit quickly",
      proc.waitFor(10, TimeUnit.SECONDS)
    )

    assertTrue(
      s"`git ls-files` stdout: <${stdout.substring(0, Math.min(100, stdout.length))}...>",
      stdout.length > 100
    )

    assertEquals(s"$prefix`git ls-files` stderr: <$stderr>", "", stderr)

    assertTrue(prefix + "`git ls-files` exited", !proc.isAlive)
    assertEquals(prefix + "`git ls-files` exit code", 0, proc.exitValue())
  }
