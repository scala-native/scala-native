 ...
 *
 * file: src/test/tck/DoubleAccumulatorTest.java
 * revision 1.9, dated: 2019-09-08
 */
...
class DoubleAccumulatorTest extends JSR166Test {
  import JSR166Test._

  @Test def testConstructor(): Unit = {
    for (identity <- List(
          jl.Double.NEGATIVE_INFINITY,
          jl.Double.POSITIVE_INFINITY,
          jl.Double.MIN_VALUE,
          jl.Double.MAX_VALUE,
          0.0
        ))
      assertEquals(
        identity,
        new DoubleAccumulator(jl.Double.max, identity).get(),
        0.0
      )
  }
  ...
  @Test def testAccumulateAndGet(): Unit = {
    ...
    acc.accumulate(2.0)
    assertEquals("a1", 2.0, acc.get(), 0.0)
    acc.accumulate(-4.0)
    assertEquals("a2", 2.0, acc.get(), 0.0)
    acc.accumulate(4.0)
  ...
  @Test def testGetThenReset(): Unit = {
    ...
    acc.accumulate(2.0)
    assertEquals("a1", 2.0, acc.get(), 0.0)
    assertEquals("a2", 2.0, acc.getThenReset(), 0.0)
  ...
  @Test def testToString(): Unit = {
    val acc = new DoubleAccumulator(jl.Double.max, 0.0)
    assertEquals("a1", "0.0", acc.toString())
    acc.accumulate(1.0)
  ...
  @Test def testAccumulateAndGetMT(): Unit = {
    ...
    usingPoolCleaner(p) { _ =>
      ...
      phaser.arriveAndAwaitAdvance()
      phaser.arriveAndAwaitAdvance()
      assertEquals(total, acc.get(), 0.0)
    }
  }
}
