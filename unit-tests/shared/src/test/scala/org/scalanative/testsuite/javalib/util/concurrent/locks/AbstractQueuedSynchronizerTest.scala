...
class AbstractQueuedSynchronizerTest extends JSR166Test {
  ...
  class Mutex extends AbstractQueuedSynchronizer {
    ...
    override def tryAcquire(acquires: Int) = {
      assertEquals(LOCKED, acquires)
      compareAndSetState(UNLOCKED, LOCKED)
    }

    override def tryRelease(releases: Int): Boolean = {
   ...
   *  AbstractQueuedSynchronizer are tested via other test classes, including
   *  those for ReentrantLock, ReentrantReadWriteLock, and Semaphore.
   *
  ...
  class BooleanLatch extends AbstractQueuedSynchronizer {
    ...
    override def tryAcquireShared(ignore: Int): Int =
      if (isSignalled()) 1 else -1
    override def tryReleaseShared(ingore: Int): Boolean = {
      setState(1)
      true
    }
  }
   ...
   */
  def waitForQueuedThread(sync: AbstractQueuedSynchronizer, t: Thread) = {
  ...
  /** JDK-8191483: AbstractQueuedSynchronizer cancel/cancel race ant
   *  -Djsr166.tckTestClass=AbstractQueuedSynchronizerTest
  ...
  @Test def testCancelCancelRace() = {
    ...
    class Sync extends AbstractQueuedSynchronizer {
      ...
      override protected def tryRelease(releases: Int): Boolean =
   ...
   *  AbstractQueuedSynchronizer when tryAcquire methods throw ant
   *  -Djsr166.tckTestClass=AbstractQueuedSynchronizerTest
  ...
  @Test def testInterruptedFailingAcquire() = {
    ...
    class Sync extends AbstractQueuedSynchronizer {
      ...
      override protected def tryAcquire(ignored: Int): Boolean = {
        maybeThrow()
        false
      }
      ...
      override def tryRelease(ignored: Int) = true
      override def tryReleaseShared(ignored: Int) = true
    }
