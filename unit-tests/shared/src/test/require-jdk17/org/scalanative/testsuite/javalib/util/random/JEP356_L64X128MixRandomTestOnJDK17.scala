...
class JEP356_L64X128MixRandomTestOnJDK17 {

  final val algorithmName = "L64X128MixRandom"
  ...
  @Test def nextBytes(): Unit = {
    ...
    val nBytes = 103 // try to trick up algorithm, use odd non-power-of-2.
  ...
  @Test def algorithmAttributes(): Unit = {
    ...
    assertEquals("equidistribution()", 2, factory.equidistribution())
    ...
    assertEquals("stateBits()", 192, factory.stateBits())
  }
