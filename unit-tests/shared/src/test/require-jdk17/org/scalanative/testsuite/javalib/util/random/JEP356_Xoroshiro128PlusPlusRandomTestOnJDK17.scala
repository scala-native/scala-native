...
class JEP356_Xoroshiro128PlusPlusRandomTestOnJDK17 {
  ...
  @Test def create_Constructor_ByteSeed(): Unit = {
    val seedSize = 2 * 8 // 1 * sizeof[Long]
  ...
  @Test def nextBytes(): Unit = {
    ...
    val nBytes = 97 // try to trick up algorithm, use odd non-power-of-2.
  ...
  @Test def nextGaussian_MeanStddev(): Unit = {
     ...
     * acceptable result. There is no good way to check against exact JVM
     * results here.
     *
  ...
  @Test def algorithmAttributes(): Unit = {
    ...
    assertEquals("group()", "Xoroshiro", factory.group())
    ...
    assertEquals("stateBits()", 128, factory.stateBits())
  }
