...
object RegexSuite extends tests.Suite {
  ...
  test("search words") { Scope { implicit in =>    
    assert(Regex.search("""\S+""", simpleText))
  }}

  test("search many times") { Scope { implicit in =>
  ...
  test("search with result") { Scope { implicit in =>
    // reusable regex
    val reusableRegex = Regex("""\w+""")
  ...
  test("matchAll with result") { Scope { implicit in =>
    ...
    // instant regex
    val result2 = Regex.matchAllWithResult("""(\s*\w+\s*)+""", simpleText).result
  ...
  test("iterator") { Scope { implicit in =>
    // reusable regex
    val reusableRegex = Regex("""\w+""")
  ...
  test("token iterator") { Scope { implicit in =>
    // reusable regex
    val reusableRegex = Regex("""\w+""")
  ...
  test("replaceAll") { Scope { implicit in =>
    // $` means prefix
    // $& means match
    // $' means suffix
    // $0..9 means groups (also \0..\99)
    assert(Regex.replaceAll("""\w+""", simpleText, "[$&]") == "[First] [second] [third]")
    assert(Regex.replaceAll("""i|e|o""", simpleText, "*") == "F*rst s*c*nd th*rd")
    assert(Regex.replaceAll("""(\w+) & (\w+)""", "cpp & scala", "$2 & $1") == "scala & cpp")
  }}

  test("replaceFirst") { Scope { implicit in =>
    // $` means prefix
    // $& means match
    // $' means suffix
    // $0..9 means groups (also \0..\99)
    assert(Regex.replaceFirst("""sec\w+""", simpleText, "$`[$&]$'") == "First First [second] third third")
  }}

  test("split") { Scope { implicit in =>
  ...
  test("ignore case") { Scope { implicit in =>
    assertNot(Regex.search("""SECOND""", simpleText))
    assert(Regex.search("""SECOND""", simpleText, Regex.icase))
  }}

  test("setting up locale") { Scope { implicit in =>
    val text = "symbol 'é' the regex [[:lower:]] imbued with a French locale should match the character"
    val re_str = "'[[:lower:]]'"
  ...
  test("format and back tracing") { Scope { implicit in =>
    ...
    val result = Regex.searchWithResult("""<([a-zA-Z][a-zA-Z0-9]*)\b[^>]*>(.*?)</\1>""",
  ...
  test("simple http url parser") { Scope { implicit in =>
    // group(0) whole URL
    // group(1) protocol HTTP/HTTPS
    // group(2) host
    // group(3) path on server and parameters
    val url_re = Regex("""(https?):\/\/((?:wwww\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,4}\b)([-a-zA-Z0-9@:%_\+.~#?&//=]*)""",
    ...
    assert(url_re.searchWithResult("""http://www.scala-native.org/en/latest/""").result.groups(2).string == "www.scala-native.org")
    assert(url_re.searchWithResult("""https://github.com/scala-native/scala-native""").result.groups(2).string == "github.com")
    val search_url = """https://www.google.com/search?q=scala+native&oq=scala+native"""
    // make sure we captured a whole url
    assert(url_re.searchWithResult(search_url).result.groups(0).string == search_url)
  }}

  test("simple ip address validator") { Scope { implicit in =>
    val url_re = Regex("""\b(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\b""",
                       Regex.optimize | // pre-compile and optimize expresion (slower creation but faster parsing)
                       Regex.icase |    // ignore case
                       Regex.no_unicode // ignore UTF-8 characters (dramatically speed-ups parsing)
                      )
  ...
  test("an issue until we will start to use C++17") { Scope { implicit in =>
