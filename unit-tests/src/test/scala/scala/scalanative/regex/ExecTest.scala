...
class ExecTest {
   ...
   * of compiler errors. Having one test means the @Ignore
   * on the test works and will show output when run as
   * part of the rest of the tests.
   *
   *
   * When the rest of the file is uncommented the tests
   * ran despite of the compile errors but slowly as noted above.
   */
  @Test def dummyTestBecauseFileWontCompileClean(): Unit = {
    assertTrue(true)
  }
  ...
  // @Test def fowlerBasic(): Unit = {
  //   testFowler("basic.dat")
  // }

  // @Test def fowlerNullSubexpr(): Unit = {
  //   testFowler("nullsubexpr.dat")
  // }

  // @Test def fowlerRepetition(): Unit = {
  ...
  //   shouldCompileMatch(0) = true
  //   shouldCompileMatch(1) = true
  //   val result = new ju.ArrayList[Integer]
  //   while (!s.isEmpty) {
  //     var end = ')'
  //     if ((result.size % 2) == 0) {
  //       if (s.charAt(0) != '(')
