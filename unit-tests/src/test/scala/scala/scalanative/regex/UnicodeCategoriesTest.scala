...
import regex.ParserTest._

//
//   Design Notes:
//
//   * UnicodeCategoriesTest.scala is derived from Parser.scala
...
//     are not validated here. Simple_Parse in ParserTest validates Braille
//     and touches Greek but coverage of others is spotty to non-extant.
//     Writing this Test revealed that the current nativelib.regex
//     implementation does not handle Properties at all!  One day, both
//     Scripts and Properties should be validated in Tests of their own.
//
...
//     makes the Test more robust to change.
//
...
class UnicodeCategoriesTest {
  ...
  // ParserTest.scala and modified to the requirements of this suite.
  // For consistency and to reduce code duplication it uses ParserTest
  ...
  private val TEST_FLAGS = ParserTest.TEST_FLAGS

  @Test def validateCodePointCountsByCategoryVersusJvmJava8Actual(): Unit = {
...
// Format: on
    ...
    assertEquals("a1", expected.length, (maxType + 1))
    assertEquals("a2", counts.length, expected.length)

    for (r <- 0 to Unicode.MAX_RUNE) {
      val t = Character.getType(r)

      assertTrue(s"Character.getType(${r}) result: ${t} not in range" +
                   s" 0 to ${maxType}",
                 (t >= 0) && (t <= maxType))
    ...
    assertTrue("Test failed, see messages", success)
  }
  ...
  @Test def punctuationPStar(): Unit = {
