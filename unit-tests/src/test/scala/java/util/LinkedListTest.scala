...
class LinkedListTest extends AbstractListTest {
  ...
  @Test def addAndRemoveProperlyInHeadAndLastPositions(): Unit = {
  ...
  @Test def shouldAddMultipleElementInOneOperation(): Unit = {
    val l  = TrivialImmutableCollection(1, 5, 2, 3, 4)
    val ll = new LinkedList[Int]()
  ...
  @Test def `couldBeInstantiatedWithPrepopulated_CollectionLinkedListTest`()
      : Unit = {
    val l  = TrivialImmutableCollection(1, 5, 2, 3, 4)
    val ll = new LinkedList[Int](l)
