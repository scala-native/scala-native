...
import java.lang.ieee754tostring.ryu.{RyuRoundingMode, RyuDouble}
...
object DoubleRyuSuite extends tests.Suite {
  ...
  test("Simple cases") {
    assertD2sEquals("0.0", 0d)
    assertD2sEquals("-0.0",
                    java.lang.Double.longBitsToDouble(0x8000000000000000L))
    assertD2sEquals("1.0", 1.0d)
    assertD2sEquals("-1.0", -1.0d)
    assertD2sEquals("NaN", java.lang.Double.NaN)
    assertD2sEquals("Infinity", java.lang.Double.POSITIVE_INFINITY)
    assertD2sEquals("-Infinity", java.lang.Double.NEGATIVE_INFINITY)
  }
