...
object US_ASCIISuite extends BaseCharset(Charset.forName("US-ASCII")) {
  test("decode") {
    // Simple tests
    ...
    testDecode(bb"48 e5 ec 6c ef")(cb"H",
                                   Malformed(1),
                                   Malformed(1),
                                   cb"l",
                                   Malformed(1))
...
//    testEncode(cb"\udbff\ud835\udcd7")(Malformed(1), Unmappable(2))
  ...
  test("isLegalReplacement") {
    ...
    assertNot(encoder.isLegalReplacement(Array(0xff.toByte)))
  }
}
