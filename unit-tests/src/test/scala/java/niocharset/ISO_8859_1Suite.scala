...
object ISO_8859_1Suite extends BaseCharset(Charset.forName("ISO-8859-1")) {
  test("decode") {
    // Simple tests
...
//    testEncode(cb"\udbff\ud835\udcd7")(Malformed(1), Unmappable(2))
  ...
  test("isLegalReplacement") {
    ...
    assert(encoder.isLegalReplacement(Array(0x80.toByte)))
    assert(encoder.isLegalReplacement(Array(0xff.toByte)))
  }
}
