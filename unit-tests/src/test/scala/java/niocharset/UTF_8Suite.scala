...
object UTF_8Suite extends BaseCharset(Charset.forName("UTF-8")) {
  test("decode") {
...
//    testEncode(cb"\udbff\ud835\udcd7")(Malformed(1), bb"f0 9d 93 97")
  ...
  test("isLegalReplacement") {
    val encoder = charset.newEncoder

    // The good ones

    assert(encoder.isLegalReplacement(Array(0x00.toByte)))
    assert(encoder.isLegalReplacement(Array(0x41.toByte)))
    assert(encoder.isLegalReplacement(Array('?'.toByte)))
    assert(encoder.isLegalReplacement(Array(0x7f.toByte)))
    ...
    assertNot(encoder.isLegalReplacement(Array(0x80.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xbf.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xc2.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xdf.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xe0.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xef.toByte)))
    assertNot(encoder.isLegalReplacement(Array(0xf4.toByte)))
