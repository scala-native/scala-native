...
class FilesTest {
  ...
  @Test def filesSetLastModifiedTimeWorksWithNanoseconds(): Unit {
    withTemporaryDirectory { dirFile =>
      val dir = dirFile.toPath()
      val f0  = dir.resolve("f0")
  ...
  @Test def filesReadAttributesWorksForNonExistentUser(): Unit = {
    if (isTravis) {
      val maxUserId =
  ...
  @Test def filesReadAttributesWorksForNonExistentGroup(): Unit = {
    if (isTravis) {
      val maxGroupId =
