...
final class PosixPattern private(_compiled: Ptr[Int], _pattern: String, _flags: Int) extends Serializable {
...
object PosixPattern {
  def compile(regex: String, flags: Int): Either[PosixRegexError, PosixPattern] = {
  ...
  def matches(regex: String, input: String): Either[PosixRegexError, Boolean] = compile(regex) match {
