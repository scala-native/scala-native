...
final class UShort private[scala] (private val underlying: Short) extends AnyVal
  ...
  @inline final def max(that: UShort): UShort = this.toUInt.max(that.toUInt).toUShort
  @inline final def min(that: UShort): UShort = this.toUInt.min(that.toUInt).toUShort
