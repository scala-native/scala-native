...
object RyuDouble {
  ...
  final val DOUBLE_EXPONENT_MASK = (1 << DOUBLE_EXPONENT_BITS) - 1

  final val DOUBLE_EXPONENT_BIAS = (1 << (DOUBLE_EXPONENT_BITS - 1)) - 1

  final val POS_TABLE_SIZE = 326
  ...
  def doubleToString(value: Double, roundingMode: RyuRoundingMode): String = {
    ...
    if (e2 >= 0) {
      ...
      val q = Math.max(0, ((e2 * 78913) >>> 18) - 1)
      ...
      val q = Math.max(0, ((-e2 * 732923) >>> 20) - 1)
      val i = -e2 - q
