...
object Match {
    ...
    private[regex] def apply(matchResult: CRegex.CRegexMatchStruct)(implicit in: Scope): Match = {
    ...
    final case class MatchResult(prefix: String, groups: Array[Group], suffix: String) {
        override def toString: String = {
            val g = groups.mkString("[", ", ", "]")
            s"{'$prefix', $g, '$suffix'}"
        }
        def ==(other:MatchResult): Boolean = {
            prefix == other.prefix && suffix == other.suffix && compareArrays(groups, other.groups)
        }
    ...
    private[regex] class MatchImpl(var matchResult: CRegex.CRegexMatchStruct)(implicit in: Scope) extends Match with Resource {
        ...
        def suffix: String = Zone { implicit z =>
            val result = stackalloc[CString]
            val result_size = stackalloc[CInt]
            CRegex.getMatchSubmatchString(matchResult, groupCount, result, result_size)
            if (!result_size>0) fromCString(!result) else ""
        }
        ...
        def format(fmt: String, flags: Match.FormatFlags = Match.format_default): String = Zone {
        implicit z =>
            val result = stackalloc[CString]
            val result_size = stackalloc[CInt]
            CRegex.matchFormat(matchResult, toCString(fmt), flags, result, result_size)
            if (!result_size>0) fromCString(!result) else ""
        }
