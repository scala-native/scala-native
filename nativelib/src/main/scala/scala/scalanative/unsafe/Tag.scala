...
object Tag {
  ...
  final case object Word extends Tag[native.Word] {
    @alwaysinline def size: Int = 8
    @alwaysinline def alignment: Int = 8
    @alwaysinline override def load(ptr: native.Ptr[native.Word]): native.Word =
      new Word(loadWord(toRawPtr(ptr)))
    @alwaysinline override def store(ptr: native.Ptr[native.Word], value: native.Word): Unit =
