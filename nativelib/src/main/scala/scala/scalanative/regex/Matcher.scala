...
final class Matcher private (private var _pattern: Pattern) {
  ...
  private def loadGroup(group: Int): Unit = {
    ...
    val end = _regionEnd.min(_lastMatchEnd + 1)
  ...
  def find(): Boolean = {
    ...
    val start =
      if (_regionStart > _lastMatchEnd) _regionStart
      else if (_lastMatchEnd > _lastMatchStart) _lastMatchEnd
      else _lastMatchEnd + 1 // empty match - nudge forward
