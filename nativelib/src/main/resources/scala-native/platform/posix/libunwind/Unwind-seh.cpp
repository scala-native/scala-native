...
_GCC_specific_handler(PEXCEPTION_RECORD ms_exc, PVOID frame, PCONTEXT ms_ctx,
    ...
    if (ms_exc->ExceptionCode == STATUS_GCC_THROW) {
        // This is (probably) a libunwind-controlled exception/unwind. Recover
        // the parameters which we set below, and pass them to the personality
        // function.
        ours = true;
    ...
    if (!ctx) {
        ...
        if (!IS_UNWINDING(ms_exc->ExceptionFlags)) {
            ...
            if (ours && ms_exc->ExceptionInformation[1] == (ULONG_PTR)frame)
                action =
                    (_Unwind_Action)(_UA_CLEANUP_PHASE | _UA_HANDLER_FRAME);
            else
    ...
    _LIBUNWIND_TRACE_UNWINDING(
        "_GCC_specific_handler() personality returned %d", urc);
    switch (urc) {
    case _URC_CONTINUE_UNWIND:
        ...
        if (action & _UA_HANDLER_FRAME)
            _LIBUNWIND_ABORT(
                "Personality continued unwind at the target frame!");
        return ExceptionContinueSearch;
    case _URC_HANDLER_FOUND:
        ...
        if (IS_UNWINDING(ms_exc->ExceptionFlags))
            _LIBUNWIND_ABORT(
                "Personality indicated exception handler in phase 2!");
...
__libunwind_seh_personality(int version, _Unwind_Action state, uint64_t klass,
                            _Unwind_Exception *exc,
    ...
    bool phase2 =
        (state & (_UA_SEARCH_PHASE | _UA_CLEANUP_PHASE)) == _UA_CLEANUP_PHASE;
...
unwind_phase2_forced(unw_context_t *uc, _Unwind_Exception *exception_object,
        ...
        if (frameInfo.handler != 0) {
            ...
            _Unwind_Reason_Code personalityResult =
                (*p)(1, action, exception_object->exception_class,
                     exception_object, (struct _Unwind_Context *)(&cursor2));
...
_Unwind_GetRegionStart(struct _Unwind_Context *context) {
    ...
    uintptr_t result =
        (uintptr_t)disp->FunctionEntry->BeginAddress + disp->ImageBase;
...
static DISPATCHER_CONTEXT *__unw_seh_get_disp_ctx(unw_cursor_t *cursor) {
    ...
    return reinterpret_cast<
               UnwindCursor<LocalAddressSpace, Registers_x86_64> *>(cursor)
...
static void __unw_seh_set_disp_ctx(unw_cursor_t *cursor,
    ...
    reinterpret_cast<UnwindCursor<LocalAddressSpace, Registers_x86_64> *>(
        cursor)
        ->setDispatcherContext(disp);
#elif defined(_LIBUNWIND_TARGET_ARM)
    reinterpret_cast<UnwindCursor<LocalAddressSpace, Registers_arm> *>(cursor)
        ->setDispatcherContext(disp);
#elif defined(_LIBUNWIND_TARGET_AARCH64)
    reinterpret_cast<UnwindCursor<LocalAddressSpace, Registers_arm64> *>(cursor)
        ->setDispatcherContext(disp);
#endif
}
