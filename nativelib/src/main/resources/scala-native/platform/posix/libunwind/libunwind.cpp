...
_LIBUNWIND_HIDDEN int __unw_set_reg(unw_cursor_t *cursor, unw_regnum_t regNum,
    ...
    if (co->validReg(regNum)) {
        ...
        // specical case altering IP to re-find info (being called by
        ...
        if (regNum == UNW_REG_IP) {
            ...
            // Normal frame unwinding would have included the offset already in
            // the CFA computation. Note: for PA-RISC and other platforms where
            // the stack grows up, this should actually be - info.gp. LLVM
            // doesn't currently support any such platforms and Clang doesn't
            // export a macro for them.
            if (info.gp)
...
_LIBUNWIND_HIDDEN int __unw_resume(unw_cursor_t *cursor) {
    _LIBUNWIND_TRACE_API("__unw_resume(cursor=%p)",
                         static_cast<void *>(cursor));
#if __has_feature(address_sanitizer) || defined(__SANITIZE_ADDRESS__)
...
void __unw_add_dynamic_fde(unw_word_t fde) {
    ...
    const char *message = CFI_Parser<LocalAddressSpace>::decodeFDE(
        ...
        &fdeInfo, &cieInfo);
    if (message == NULL) {
        // dynamically registered FDEs don't have a mach_header group they are
        // in. Use fde as mh_group
        unw_word_t mh_group = fdeInfo.fdeStart;
        DwarfFDECache<LocalAddressSpace>::add(
            (LocalAddressSpace::pint_t)mh_group, fdeInfo.pcStart, fdeInfo.pcEnd,
            fdeInfo.fdeStart);
    } else {
