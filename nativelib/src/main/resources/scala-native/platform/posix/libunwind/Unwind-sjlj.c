    ...
    for (bool handlerNotFound = true; handlerNotFound; c = c->prev) {
        ...
        if (c->personality != NULL) {
            ...
            case _URC_HANDLER_FOUND:
                ...
                // frame stop search and remember function context
        ...
        if (c->personality != NULL) {
            ...
            if ((uintptr_t)c == exception_object->private_2)
                action = (_Unwind_Action)(
                    _UA_CLEANUP_PHASE |
                    _UA_HANDLER_FRAME); // tell personality this was the frame
                                        // it marked in phase 1
            _Unwind_Reason_Code personalityResult = (*c->personality)(
...
_Unwind_SjLj_Resume_or_Rethrow(struct _Unwind_Exception *exception_object) {
    ...
    if (exception_object->private_1 == 0) {
        ...
        // should return if there is no catch clause, so that __cxa_rethrow can
        // call std::terminate()
    }
