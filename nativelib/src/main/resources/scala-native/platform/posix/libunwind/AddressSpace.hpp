...
static inline bool _dyld_find_unwind_sections(void *addr,
    ...
    info->dwarf_section =
        getsectiondata(mh, "__TEXT", "__eh_frame", &info->dwarf_section_length);
    info->compact_unwind_section = getsectiondata(
        mh, "__TEXT", "__unwind_info", &info->compact_unwind_section_length);
...
// The following linker script may be used to produce the necessary sections and
// symbols. Unless the --eh-frame-hdr linker option is provided, the section is
// not generated and does not take space in the output file.
//
//   .eh_frame :
//   {
//       __eh_frame_start = .;
...
LocalAddressSpace::getEncodedP(pint_t &addr, pint_t end, uint8_t encoding,
    ...
    case DW_EH_PE_datarel:
        ...
        // a default value of 0, and we abort in the event that someone calls
...
inline bool LocalAddressSpace::findUnwindSections(pint_t targetAddr,
    ...
    info.dwarf_section_length =
        (uintptr_t)(&__eh_frame_end - &__eh_frame_start);
    ...
    info.dwarf_index_section_length =
        (uintptr_t)(&__eh_frame_hdr_end - &__eh_frame_hdr_start);
    ...
    _LIBUNWIND_TRACE_UNWINDING("findUnwindSections: section %p length %p",
                               (void *)info.arm_section,
                               (void *)info.arm_section_length);
    ...
    for (unsigned i = 0; i < (needed / sizeof(HMODULE)); i++) {
        ...
        PIMAGE_NT_HEADERS pinh =
            (PIMAGE_NT_HEADERS)((BYTE *)pidh + pidh->e_lfanew);
    ...
    "_LIBUNWIND_SUPPORT_DWARF_UNWIND requires _LIBUNWIND_SUPPORT_DWARF_INDEX on this platform."
            ...
            for (Elf_Half i = 0; i < pinfo->dlpi_phnum; i++) {
                ...
                if (phdr->p_type == PT_LOAD) {
                    ...
                    if (cbdata->targetAddr >= begin &&
                        cbdata->targetAddr < end) {
                    ...
                    uintptr_t eh_frame_hdr_start =
                        pinfo->dlpi_addr + phdr->p_vaddr;
#if defined(__ANDROID__)
                    if (pinfo->dlpi_addr == 0 && phdr->p_vaddr < image_base)
...
template <typename P> class RemoteAddressSpace {
  public:
...
uint64_t OtherAddressSpace<P>::getULEB128(pint_t &addr, pint_t end) {
    ...
    LocalAddressSpace::pint_t laddr =
        (LocalAddressSpace::pint_t)localCopy(addr);
