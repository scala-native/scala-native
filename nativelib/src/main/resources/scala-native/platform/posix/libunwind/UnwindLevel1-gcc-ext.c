...
_Unwind_Resume_or_Rethrow(_Unwind_Exception *exception_object) {
    ...
    if (exception_object->PRIVATE_1 == 0) {
        ...
        // Will return if there is no catch clause, so that __cxa_rethrow can
        // call std::terminate().
    }
    ...
    _LIBUNWIND_ABORT(
        "_Unwind_Resume_or_Rethrow() called _Unwind_RaiseException()"
    ...
    while (true) {
        ...
        if (__unw_step(&cursor) <= 0) {
            _LIBUNWIND_TRACE_UNWINDING(
                " _backtrace: ended because cursor reached "
        ...
        _Unwind_Personality_Fn handler =
            (_Unwind_Personality_Fn)frameInfo.handler;
        if (handler == NULL) {
            return _URC_END_OF_STACK;
        }
        if (handler(_US_VIRTUAL_UNWIND_FRAME | _US_FORCE_UNWIND, &ex,
                    context) != _URC_CONTINUE_UNWIND) {
            return _URC_END_OF_STACK;
        }
        ...
        if (_LIBUNWIND_TRACING_UNWINDING) {
            ...
            _LIBUNWIND_TRACE_UNWINDING(
                " _backtrace: start_ip=0x%" PRIxPTR
                ", func=%s, lsda=0x%" PRIxPTR ", context=%p",
                frame.start_ip, functionName, frame.lsda, (void *)&cursor);
        }
...
_LIBUNWIND_EXPORT uintptr_t _Unwind_GetIPInfo(struct _Unwind_Context *context,
    ...
    // the implementation that hard-coded zero in every case, even signal
    // frames.
    if (isSignalFrame <= 0)
        *ipBefore = 0;
    else
...
_LIBUNWIND_EXPORT void __register_frame_info_bases(const void *fde, void *ob,
    ...
    (void)tb;
    (void)db;
    _LIBUNWIND_TRACE_API("__register_frame_info_bases(%p,%p, %p, %p)", fde, ob,
                         tb, db);
