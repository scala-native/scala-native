...
bool CFI_Parser<A>::parseInstructions(A &addressSpace, pint_t instructions,
    ...
    while ((p < instructionsEnd) && (codeOffset < pcoffset)) {
        ...
        case DW_CFA_remember_state:
            ...
            entry =
                (PrologInfoStackEntry *)malloc(sizeof(PrologInfoStackEntry));
            if (entry != NULL) {
        ...
        case DW_CFA_val_expression:
            reg = addressSpace.getULEB128(p, instructionsEnd);
            if (reg > kMaxRegisterNumber) {
                _LIBUNWIND_LOG0("malformed DW_CFA_val_expression DWARF unwind, "
                                "reg too big");
                return false;
            }
        ...
        default:
            ...
            case DW_CFA_offset:
                ...
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG(
                        "malformed DW_CFA_offset DWARF unwind, reg (%" PRIu64
                        ") out of range",
                        reg);
                    return false;
                }
