...
///
template <typename A> class CFI_Parser {
  public:
    ...
    enum { kMaxRegisterNumber = _LIBUNWIND_HIGHEST_DWARF_REGISTER };
    ...
    struct PrologInfo {
        ...
        void restoreRegisterToInitialState(uint64_t reg,
                                           PrologInfo &initialState) {
            if (savedRegisters[reg].initialStateSaved)
    ...
    struct RememberStack {
        ...
        ~RememberStack() {
            ...
            // parseInstructions can skip restore opcodes if it reaches the
            // target PC and stops interpreting, so we have to make sure we
            // don't leak memory.
            while (entry) {
...
const char *CFI_Parser<A>::decodeFDE(A &addressSpace, pint_t fdeStart,
    ...
    if (cieInfo->fdesHaveAugmentationData) {
        ...
        if (cieInfo->lsdaEncoding != DW_EH_PE_omit) {
            ...
            pint_t lsdaStart = p;
            if (addressSpace.getEncodedP(p, nextCFI,
                                         cieInfo->lsdaEncoding & 0x0F) != 0) {
        ...
        if (id == 0) {
            ...
            if ((ehSectionStart <= cieStart) && (cieStart < ehSectionEnd)) {
                if (parseCIE(addressSpace, cieStart, cieInfo) == NULL) {
                    p += 4;
                    // Parse pc begin and range.
                    pint_t pcStart = addressSpace.getEncodedP(
                    ...
                    if ((pcStart < pc) && (pc <= pcStart + pcRange)) {
                        ...
                        if (cieInfo->fdesHaveAugmentationData) {
                            ...
                            pint_t endOfAug = p + augLen;
                            if (cieInfo->lsdaEncoding != DW_EH_PE_omit) {
                                // Peek at value (without indirection).  Zero
                                // means no LSDA.
                                pint_t lsdaStart = p;
                                if (addressSpace.getEncodedP(
                                    ...
                                    // Reset pointer and re-parse LSDA address.
                                    p = lsdaStart;
                                    fdeInfo->lsda = addressSpace.getEncodedP(
                                        p, nextCFI, cieInfo->lsdaEncoding);
                                }
                            }
                            p = endOfAug;
                        }
                    ...
                    // Malformed CIE, now augmentation describing pc range
                    // encoding.
                }
            } else {
...
const char *CFI_Parser<A>::parseCIE(A &addressSpace, pint_t cie,
    ...
    cieInfo->codeAlignFactor =
        (uint32_t)addressSpace.getULEB128(p, cieContentEnd);
...
bool CFI_Parser<A>::parseFDEInstructions(A &addressSpace,
                                         ...
                                         const CIE_Info &cieInfo, pint_t upToPC,
                                         int arch, PrologInfo *results) {
    // Alloca is used for the allocation of the rememberStack entries. It
    // removes the dependency on new/malloc but the below for loop can not be
    // refactored into functions. Entry could be saved during the processing of
    // a CIE and restored by an FDE.
        ...
        while ((p < instructionsEnd) && (codeOffset < pcoffset)) {
            ...
            case DW_CFA_advance_loc1:
                ...
                _LIBUNWIND_TRACE_DWARF(
                    "DW_CFA_advance_loc1: new offset=%" PRIu64 "\n",
            ...
            case DW_CFA_advance_loc2:
                ...
                p += 2;
                _LIBUNWIND_TRACE_DWARF(
                    "DW_CFA_advance_loc2: new offset=%" PRIu64 "\n",
                    static_cast<uint64_t>(codeOffset));
                break;
            case DW_CFA_advance_loc4:
                ...
                p += 4;
                _LIBUNWIND_TRACE_DWARF(
                    "DW_CFA_advance_loc4: new offset=%" PRIu64 "\n",
                    static_cast<uint64_t>(codeOffset));
                break;
            ...
            case DW_CFA_restore_extended:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_restore_extended DWARF "
                                    "unwind, reg too big");
                    return false;
                }
                results->restoreRegisterToInitialState(reg, initialState);
                _LIBUNWIND_TRACE_DWARF(
                    "DW_CFA_restore_extended(reg=%" PRIu64 ")\n", reg);
                break;
            case DW_CFA_undefined:
                ...
                results->setRegisterLocation(reg, kRegisterUndefined,
                                             initialState);
            ...
            case DW_CFA_same_value:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_same_value DWARF unwind, "
                                    "reg too big");
                    return false;
                }
                // <rdar://problem/8456377> DW_CFA_same_value unsupported
                // "same value" means register was stored in frame, but its
                // current value has not changed, so no need to restore from
                // frame. We model this as if the register was never saved.
                results->setRegisterLocation(reg, kRegisterUnused,
                                             initialState);
            ...
            case DW_CFA_register:
                ...
                results->setRegister(reg, kRegisterInRegister, (int64_t)reg2,
                                     initialState);
                _LIBUNWIND_TRACE_DWARF("DW_CFA_register(reg=%" PRIu64
                                       ", reg2=%" PRIu64 ")\n",
                                       reg, reg2);
                break;
            case DW_CFA_remember_state: {
                // Avoid operator new because that would be an upward
                // dependency. Avoid malloc because it needs heap allocation.
                PrologInfoStackEntry *entry =
            ...
            case DW_CFA_def_cfa:
                ...
                _LIBUNWIND_TRACE_DWARF("DW_CFA_def_cfa(reg=%" PRIu64
                                       ", offset=%" PRIu64 ")\n",
                                       reg, offset);
                break;
            case DW_CFA_def_cfa_register:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_def_cfa_register DWARF "
                                    "unwind, reg too big");
                    return false;
                }
                results->cfaRegister = (uint32_t)reg;
            ...
            case DW_CFA_def_cfa_expression:
                ...
                _LIBUNWIND_TRACE_DWARF(
                    "DW_CFA_def_cfa_expression(expression=0x%" PRIx64
                    ", length=%" PRIu64 ")\n",
            ...
            case DW_CFA_expression:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_expression DWARF unwind, "
                                    "reg too big");
                    return false;
                }
            ...
            case DW_CFA_offset_extended_sf:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_offset_extended_sf DWARF "
                                    "unwind, reg too big");
                    return false;
                }
            ...
            case DW_CFA_def_cfa_sf:
                ...
                offset = addressSpace.getSLEB128(p, instructionsEnd) *
                         cieInfo.dataAlignFactor;
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_def_cfa_sf DWARF unwind, "
                                    "reg too big");
                    return false;
                }
                results->cfaRegister = (uint32_t)reg;
            ...
            case DW_CFA_val_offset:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG("malformed DW_CFA_val_offset DWARF unwind, "
                                   "reg (%" PRIu64 ") out of range\n",
                                   reg);
                    return false;
                }
                offset = (int64_t)addressSpace.getULEB128(p, instructionsEnd) *
                         cieInfo.dataAlignFactor;
                results->setRegister(reg, kRegisterOffsetFromCFA, offset,
                                     initialState);
            ...
            case DW_CFA_val_offset_sf:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_val_offset_sf DWARF "
                                    "unwind, reg too big");
                    return false;
                }
                offset = addressSpace.getSLEB128(p, instructionsEnd) *
            ...
            case DW_CFA_val_expression:
                reg = addressSpace.getULEB128(p, instructionsEnd);
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0("malformed DW_CFA_val_expression DWARF "
                                    "unwind, reg too big");
                    return false;
                }
            ...
            case DW_CFA_GNU_negative_offset_extended:
                ...
                if (reg > kMaxRegisterNumber) {
                    _LIBUNWIND_LOG0(
                        "malformed DW_CFA_GNU_negative_offset_extended DWARF "
                        "unwind, reg too big");
                    return false;
                }
                offset = (int64_t)addressSpace.getULEB128(p, instructionsEnd) *
                         cieInfo.dataAlignFactor;
                results->setRegister(reg, kRegisterInCFA, -offset,
                                     initialState);
                _LIBUNWIND_TRACE_DWARF(
                ...
                // The same constant is used to represent different instructions
                // on AArch64 (negate_ra_state) and SPARC (window_save).
                static_assert(DW_CFA_AARCH64_negate_ra_state ==
            ...
            case DW_CFA_AARCH64_negate_ra_state:
                ...
                case REGISTERS_ARM64: {
                    int64_t value =
                        results->savedRegisters[UNW_ARM64_RA_SIGN_STATE].value ^
                        0x1;
            ...
            default:
                ...
                case DW_CFA_offset:
                    reg = operand;
                    if (reg > kMaxRegisterNumber) {
                        _LIBUNWIND_LOG("malformed DW_CFA_offset DWARF unwind, "
                                       "reg (%" PRIu64 ") out of range",
                                       reg);
                        return false;
                    }
                    offset =
                        (int64_t)addressSpace.getULEB128(p, instructionsEnd) *
                        cieInfo.dataAlignFactor;
                    results->setRegister(reg, kRegisterInCFA, offset,
                                         initialState);
                    _LIBUNWIND_TRACE_DWARF(
                ...
                case DW_CFA_advance_loc:
                    ...
                    _LIBUNWIND_TRACE_DWARF(
                        "DW_CFA_advance_loc: new offset=%" PRIu64 "\n",
                        static_cast<uint64_t>(codeOffset));
                    break;
                case DW_CFA_restore:
                    reg = operand;
                    if (reg > kMaxRegisterNumber) {
                        _LIBUNWIND_LOG("malformed DW_CFA_restore DWARF unwind, "
                                       "reg (%" PRIu64 ") out of range",
                                       reg);
                        return false;
                    }
