...
class _LIBUNWIND_HIDDEN FrameHeaderCache {
    ...
    // recently used entries.  FIXME: Would the the extra memory for a
    // doubly-linked list be better than the runtime cost of traversing a very
    // short singly-linked list on a cache miss? The entries themselves are all
    // small and consecutive, so unlikely to cause page faults when following
    // the pointers. The memory spent on additional pointers could also be spent
    // on more entries.
    ...
    bool cacheNeedsReset(dl_phdr_info *PInfo) {
        ...
        // loading and unloading shared libraries. If these values change
        // between iterations of dl_iterate_phdr, then invalidate the cache.

        // These are static to avoid needing an initializer, and unsigned long
        // long because that is their type within the extended dl_phdr_info.
        // Initialize these to something extremely unlikely to be found upon the
        // first call to dl_iterate_phdr.
        ...
        if (PInfo->dlpi_adds != LastAdds || PInfo->dlpi_subs != LastSubs) {
            // Resetting the entire cache is a big hammer, but this path is
            // rare-- usually just on the very first call, when the cache is
            // empty anyway--so added complexity doesn't buy much.
  ...
  public:
    bool find(dl_phdr_info *PInfo, size_t, void *data) {
        ...
        while (Current != nullptr) {
            ...
            if (Current->LowPC() <= CBData->targetAddr &&
                ...
                _LIBUNWIND_FRAMEHEADERCACHE_TRACE(
                    "FrameHeaderCache hit %lx in [%lx - %lx)",
                    CBData->targetAddr, Current->LowPC(), Current->HighPC());
                if (Previous) {
        ...
        _LIBUNWIND_FRAMEHEADERCACHE_TRACE(
            "FrameHeaderCache miss for address %lx", CBData->targetAddr);
        return false;
    }
    ...
    void add(const UnwindInfoSections *UIS) {
        ...
        if (Unused != nullptr) {
            ...
            _LIBUNWIND_FRAMEHEADERCACHE_TRACE(
                "FrameHeaderCache evict [%lx - %lx)", Current->LowPC(),
                Current->HighPC());
        }
