...
template <typename A> class _LIBUNWIND_HIDDEN DwarfFDECache {
  ...
  public:
    ...
    static void
    iterateCacheEntries(void (*func)(unw_word_t ip_start, unw_word_t ip_end,
...
template <typename A>
typename DwarfFDECache<A>::entry *DwarfFDECache<A>::_buffer = _initialBuffer;

template <typename A>
typename DwarfFDECache<A>::entry *DwarfFDECache<A>::_bufferUsed =
    _initialBuffer;

template <typename A>
typename DwarfFDECache<A>::entry *DwarfFDECache<A>::_bufferEnd =
    &_initialBuffer[64];

template <typename A>
typename DwarfFDECache<A>::entry DwarfFDECache<A>::_initialBuffer[64];

template <typename A> RWMutex DwarfFDECache<A>::_lock;

#ifdef __APPLE__
template <typename A> bool DwarfFDECache<A>::_registeredForDyldUnloads = false;
#endif
  ...
  public:
    ...
    uint32_t version() const {
        return _addressSpace.get32(
            _addr + offsetof(unwind_info_section_header, version));
    }
...
template <typename A> class UnwindSectionRegularPageHeader {
  public:
    ...
    uint32_t kind() const {
        return _addressSpace.get32(
            _addr +
            offsetof(unwind_info_regular_second_level_page_header, kind));
    }
...
class _LIBUNWIND_HIDDEN AbstractUnwindCursor {
  public:
    ...
    virtual void saveVFPAsX() {
        _LIBUNWIND_ABORT("saveVFPAsX not implemented");
    }
#endif
};
...
class UnwindCursor : public AbstractUnwindCursor {
  ...
  private:
    ...
    int stepWithEHABI() {
        ...
        // abstraction layer.
        const uint32_t *ehtp = decode_eht_entry(
            reinterpret_cast<const uint32_t *>(_info.unwind_info), &off, &len);
    ...
    int stepWithCompactEncoding(Registers_ppc &) { return UNW_EINVAL; }
#endif

#if defined(_LIBUNWIND_TARGET_PPC64)
    int stepWithCompactEncoding(Registers_ppc64 &) { return UNW_EINVAL; }
#endif
    ...
    int stepWithCompactEncoding(Registers_mips_o32 &) { return UNW_EINVAL; }
#endif
    ...
    bool compactSaysUseDwarf(Registers_x86_64 &, uint32_t *offset) const {
        if ((_info.format & UNWIND_X86_64_MODE_MASK) ==
...
#if defined(_LIBUNWIND_TARGET_PPC)
    bool compactSaysUseDwarf(Registers_ppc &, uint32_t *) const { return true; }
#endif
    ...
    bool compactSaysUseDwarf(Registers_arm64 &, uint32_t *offset) const {
        if ((_info.format & UNWIND_ARM64_MODE_MASK) ==
    ...
    compact_unwind_encoding_t dwarfEncoding(Registers_arm &) const { return 0; }
#endif
...
template <typename A> struct EHABISectionIterator {
    ...
    typename A::pint_t functionAddress() const {
        typename A::pint_t indexAddr =
            _sects->arm_section +
            arrayoffsetof(EHABIIndexEntry, _i, functionOffset);
        return indexAddr + signExtendPrel31(_addressSpace->get32(indexAddr));
    }

    typename A::pint_t dataAddress() {
        typename A::pint_t indexAddr =
            _sects->arm_section + arrayoffsetof(EHABIIndexEntry, _i, data);
        return indexAddr;
    }
...
bool UnwindCursor<A, R>::getInfoFromEHABISection(
    ...
    // If an exception is thrown from a function, corresponding to the last
    ...
    // inside the index table entry on the second word (aka |indexDataAddr|).
    // Otherwise, the table points at an offset in the exception handling table
    // (section 5 EHABI).
    ...
    // If the high bit in the exception handling table entry is set, the entry
    // is in compact form (section 6.3 EHABI).
...
#endif
                    DwarfFDECache<A>::add(sects.dso_base, fdeInfo.pcStart,
                                          fdeInfo.pcEnd, fdeInfo.fdeStart);
            }
            return true;
        }
    }
...
bool UnwindCursor<A, R>::getInfoFromCompactEncodingSection(
    ...
    const UnwindSectionIndexArray<A> topIndex(
        _addressSpace,
        sects.compact_unwind_section + sectionHeader.indexSectionOffset());
    uint32_t low = 0;
    uint32_t high = sectionHeader.indexCount();
    uint32_t last = high - 1;
    while (low < high) {
        uint32_t mid = (low + high) / 2;
        // if ( log ) fprintf(stderr, "\tmid=%d, low=%d, high=%d,
    ...
    const pint_t secondLevelAddr = sects.compact_unwind_section +
                                   topIndex.secondLevelPagesSectionOffset(low);
    const pint_t lsdaArrayStartAddr = sects.compact_unwind_section +
                                      topIndex.lsdaIndexArraySectionOffset(low);
    const pint_t lsdaArrayEndAddr =
        sects.compact_unwind_section +
        topIndex.lsdaIndexArraySectionOffset(low + 1);
    if (log)
        fprintf(stderr,
    ...
    if (pageKind == UNWIND_SECOND_LEVEL_REGULAR) {
        ...
        while (low < high) {
            ...
            if (pageIndex.functionOffset(mid) <= targetFunctionOffset) {
                if (mid == (uint32_t)(pageHeader.entryCount() - 1)) {
                    ...
                    break;
                } else if (pageIndex.functionOffset(mid + 1) >
                           targetFunctionOffset) {
                    // next is too big, so we found it
                    low = mid;
                    funcEnd =
                        pageIndex.functionOffset(low + 1) + sects.dso_base;
                    break;
                } else {
                    low = mid + 1;
                }
        ...
        while (low < high) {
            ...
            if (pageIndex.functionOffset(mid) <= targetFunctionPageOffset) {
                if ((mid == last) || (pageIndex.functionOffset(mid + 1) >
        ...
        funcStart = pageIndex.functionOffset(low) + firstLevelFunctionOffset +
                    sects.dso_base;
        if (low < last)
            funcEnd = pageIndex.functionOffset(low + 1) +
        ...
        if (encodingIndex < sectionHeader.commonEncodingsArrayCount()) {
            ...
            uint16_t pageEncodingIndex =
                encodingIndex -
                (uint16_t)sectionHeader.commonEncodingsArrayCount();
            encoding = _addressSpace.get32(
                secondLevelAddr + pageHeader.encodingsPageOffset() +
    ...
    if (encoding & UNWIND_HAS_LSDA) {
        ...
        if (log)
            fprintf(stderr,
                    "\tbinary search of lsda table for "
                    "targetFunctionOffset=0x%08X\n",
                    funcStartOffset);
        while (low < high) {
            uint32_t mid = (low + high) / 2;
        ...
        if (lsda == 0) {
            _LIBUNWIND_DEBUG_LOG(
                "found encoding 0x%08X with HAS_LSDA bit set for "
        ...
        if (sects.compact_unwind_section != 0) {
            if (this->getInfoFromCompactEncodingSection(pc, sects)) {
                ...
                if ((sects.dwarf_section != 0) &&
                    compactSaysUseDwarf(&dwarfOffset)) {
...
#endif
                // If unwind table has entry, but entry says there is no unwind
                // info, record that we have no unwind info.
                if (_info.format == 0)
...
bool UnwindCursor<A, R>::getFunctionName(char *buf, size_t bufLen,
                                         unw_word_t *offset) {
    return _addressSpace.findFunctionName((pint_t)this->getReg(UNW_REG_IP), buf,
                                          bufLen, offset);
}
