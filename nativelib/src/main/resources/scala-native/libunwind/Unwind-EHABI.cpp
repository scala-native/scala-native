...
ProcessDescriptors(_Unwind_State state, _Unwind_Control_Block *ucbp,
    ...
    while (descriptorWord) {
        ...
        // See # 9.2 table for decoding the kind of descriptor. It's a 2-bit
        // value.
        Descriptor::Kind kind = static_cast<Descriptor::Kind>(
            (length & 0x1) | ((offset & 0x1) << 1));
            ...
            if (isInScope) {
                ...
                if (landing_pad == 0xffffffff) {
                    ...
                    if (__cxxabiv1::__cxa_type_match(
                            ucbp, reinterpret_cast<const std::type_info
                    *>(landing_pad), is_reference_type, &matched_object) !=
                    __cxxabiv1::ctm_failed) return _URC_HANDLER_FOUND;
                        */
...
static _Unwind_Reason_Code unwindOneFrame(_Unwind_State state,
    ...
    const char *lsda = reinterpret_cast<const char *>(
        ...
        if ((byte & 0x80) == 0) {
            ...
            case 0xb0: {
                ...
                case 0xb1: {
                    ...
                    if (registers & 0xf0 || !registers)
                        return _URC_FAILURE;
                    _Unwind_VRS_Pop(context, _UVRSC_CORE, registers,
                                    _UVRSD_UINT32);
                    break;
                }
                case 0xb2: {
            ...
            case 0xd0: {
                if (byte & 0x08)
                    return _URC_FAILURE;
                _Unwind_VRS_Pop(context, _UVRSC_VFP,
                                RegisterRange(8, byte & 0x7), _UVRSD_DOUBLE);
                break;
            }
            default:
                return _URC_FAILURE;
...
static _Unwind_Reason_Code unwind_phase1(unw_context_t *uc,
    ...
    for (bool handlerNotFound = true; handlerNotFound;) {
        ...
        if (frameInfo.handler != 0) {
            ...
            _LIBUNWIND_TRACE_UNWINDING(
                "unwind_phase1(ex_ojb=%p): personality result %d start_ip %x "
                "ehtp %p "
                "additional %x",
            ...
            case _URC_HANDLER_FOUND:
                ...
                // frame stop search and remember stack pointer at the frame
...
static _Unwind_Reason_Code unwind_phase2(unw_context_t *uc,
    ...
    while (true) {
        ...
        if (resume && frame_count == 1) {
            // On a resume, first unwind the _Unwind_Resume() frame. The next
            // frame is now the landing pad for the cleanup from a previous
            // execution of phase2. To continue unwindingly correctly, replace
            // VRS[15] with the IP of the frame that the previous run of phase2
            // installed the context for. After this, continue unwinding as if
            // normal.
            //
        ...
        if (frameInfo.handler != 0) {
            ...
            case _URC_CONTINUE_UNWIND:
                ...
                if (sp == exception_object->barrier_cache.sp) {
                    ...
                    // not...
                    _LIBUNWIND_ABORT(
                        "during phase1 personality function said it would "
            ...
            default:
                ...
                _LIBUNWIND_DEBUG_LOG(
                    "personality function returned unknown result %d",
                    personalityResult);
                return _URC_FATAL_PHASE2_ERROR;
            }
        }
...
_LIBUNWIND_EXPORT void _Unwind_Complete(_Unwind_Exception *exception_object) {
    // This is to be called when exception handling completes to give us a
    // chance to perform any housekeeping. EHABI #7.2. But we have nothing to do
    // here.
...
_LIBUNWIND_EXPORT _Unwind_VRS_Result _Unwind_VRS_Get(
    _Unwind_Context *context, _Unwind_VRS_RegClass regclass, uint32_t regno,
    _Unwind_VRS_DataRepresentation representation, void *valuep) {
    _Unwind_VRS_Result result = _Unwind_VRS_Get_Internal(
        context, regclass, regno, representation, valuep);
