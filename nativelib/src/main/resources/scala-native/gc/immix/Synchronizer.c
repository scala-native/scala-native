...
/*
 * Continuation migration may leave a previous carrier's TLS-derived trap
 * cell pointer cached in the resumed function's stack/registers (LLVM is
 * free to CSE `mrs tpidr_el0` and the subsequent TLS-slot dereference
...
static void Synchronizer_WaitForResumption(MutatorThread *selfThread) {
    ...
    if (WAIT_OBJECT_0 !=
        ...
        GC_LOG_WARN("suspend thread failed");
        exit(GetLastError());
    }
#else
    int signum;
...
#endif // SCALANATIVE_MULTITHREADING_ENABLED && SCALANATIVE_GC_IMMIX
