/*
 * getMemorySize / getFreeMemorySize originate from:
 *
 ...
 *
 * getPageSize is a small Scala Native addition (see header).
 */
...
uintptr_t getPageSize(void) {
    ...
    SYSTEM_INFO si;
    GetSystemInfo(&si);
    cached = (uintptr_t)si.dwPageSize;
#else
    long ps = sysconf(_SC_PAGESIZE);
    cached = (ps > 0) ? (uintptr_t)ps : (uintptr_t)4096;
#endif
    return cached;
}
