...
void setAccessFlagHelper(ACCESS_MASK* accessRights, DWORD current, DWORD mode, DWORD testflags, DWORD flags)
{
...
void setChmodFlagHelper(mode_t* chmodmode, ACCESS_MASK accessRights, DWORD testflags, DWORD flags)
{
...
mode_t AccessMaskToMode(ACCESS_MASK Mask)
{
    ...
    setChmodFlagHelper(&result, Mask, FILE_READ_DATA, S_ISUID | S_ISGID | S_ISVTX | S_IRUSR | S_IRGRP | S_IROTH);
    setChmodFlagHelper(&result, Mask, FILE_WRITE_DATA, S_ISUID | S_ISGID | S_ISVTX | S_IWUSR | S_IWGRP | S_IWOTH);
    setChmodFlagHelper(&result, Mask, FILE_EXECUTE, S_ISUID | S_ISGID | S_ISVTX | S_IXUSR | S_IXGRP | S_IXOTH);
...
extern "C" mode_t getAccessMode(const char * path)
{
    ...
    int res = getAccessPermissions(pathw, SE_FILE_OBJECT,
        userNameBuffer,
        TRUSTEE_IS_NAME,
        TRUSTEE_IS_USER,
        &accessRights);
...
extern "C" int pchmod(const char * path, mode_t mode)
{
    ...
    setAccessFlagHelper(&newAcessingRights, currentmode, mode, S_IRUSR, FILE_READ_DATA);
    setAccessFlagHelper(&newAcessingRights, currentmode, mode, S_IWUSR, FILE_WRITE_DATA);
    setAccessFlagHelper(&newAcessingRights, currentmode, mode, S_IXUSR, FILE_EXECUTE);
