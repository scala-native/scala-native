...
void convert_address(struct sockaddr *raw_in,
    ...
    int convert_result = scalanative_convert_scalanative_sockaddr(
        raw_in, converted_address, &len);
    if (convert_result != 0) {
...
void scalanative_convert_ifaddrs(struct ifaddrs *in,
    ...
    while (next != NULL && next->ifa_addr != NULL &&
...
int scalanative_getifaddrs(struct scalanative_ifaddrs **ifap) {
    ...
    while (ifaddr != NULL && ifaddr->ifa_addr != NULL &&
