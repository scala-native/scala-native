...
typedef void(*scalanative_misc_greenthreads_func)(scalanative_misc_greenthreads_threadId);
...
extern "C" void scalanative_misc_greenthreads_gtswtch(scalanative_misc_greenthreads_gt::gtctx *oldt, scalanative_misc_greenthreads_gt::gtctx *newt);
extern "C" bool scalanative_misc_greenthreads_gtyield(void);
extern "C" void scalanative_misc_greenthreads_gtstop(void);
extern "C" int scalanative_misc_greenthreads_gtgo(scalanative_misc_greenthreads_threadId id, scalanative_misc_greenthreads_func f);

extern "C"
void __attribute__((noinline)) scalanative_misc_greenthreads_gtinit(void)
{
    ...
    gtcur->f = 0;
    gtcur->stackSize = 0;
    gtqueue = (scalanative_misc_greenthreads_gt*)malloc(sizeof(scalanative_misc_greenthreads_gt)*MaxGTQueue);
...
extern "C"
void __attribute__ ((noreturn)) __attribute__ ((noinline)) scalanative_misc_greenthreads_gtret(int ret)
{
	if (gtcur != &gttbl[0]) {
...
extern "C"
void scalanative_misc_greenthreads_pop(scalanative_misc_greenthreads_gt *p, scalanative_misc_greenthreads_gt *q)
{
    memset(p->stack, StackSize, 0);

    if (q->stackSize == 0)
    {
        *(uint64_t *)&p->stack[StackSize - 8] = (uint64_t)scalanative_misc_greenthreads_gtstop;
...
extern "C"
bool __attribute__((noinline)) scalanative_misc_greenthreads_gtyield(void)
{
    if (!gtcur)
    {
        return false;
    ...
    for (p = &gttbl[1]; p != &gttbl[MaxGThreads] && (gtqueue && gtqueue[0].st == scalanative_misc_greenthreads_gt::Ready); p++)
    {
...
extern "C"
void __attribute__((noinline)) scalanative_misc_greenthreads_gtstop(void) { scalanative_misc_greenthreads_gtret(0); }

extern "C"
int __attribute__((noinline)) scalanative_misc_greenthreads_gtgo(scalanative_misc_greenthreads_threadId id, scalanative_misc_greenthreads_func f)
{
...
extern "C"
int __attribute__((noinline)) scalanative_misc_greenthreads_fork(scalanative_misc_greenthreads_threadId id)
{
    if (!gtcur)
    {
...
extern "C" void __attribute__((noinline)) scalanative_misc_greenthreads_gtswtch(scalanative_misc_greenthreads_gt::gtctx *oldt, scalanative_misc_greenthreads_gt::gtctx *newt)
{
#ifdef _WIN32
    asm("addq $16, %rsp");
...
extern "C"
scalanative_misc_greenthreads_threadId scalanative_misc_greenthreads_currentthreadid()
{
    return gtcur? gtcur->id : 0;
}

extern "C"
void scalanative_misc_greenthreads_join(scalanative_misc_greenthreads_threadId id)
{
    scalanative_misc_greenthreads_gt *p = 0;
    bool wait = false;
    do {
        ...
        for (p = &gttbl[1]; p != &gttbl[MaxGThreads]; p++)
        {
            if (p->id == id && p->st != scalanative_misc_greenthreads_gt::Unused)
            {
                wait = true;
                break;
            }
